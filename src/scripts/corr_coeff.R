...
#x <- data.frame("umi" = umi_total_count, "filter" = filter_total_count, "md"= md_total_count)
#df <- data.frame(x = c(umi_total_count,md_total_count,filter_total_count))
#rownames(df) <-c("umi","md","filter")
barplot(c(filter_total_count, umi_total_count, md_total_count),names.arg=c("Filter", "UMI", "DUP"), las = 1, xpd = FALSE,  col=brewer.pal(3,"Paired"), ylim=c(md_total_count - 1000000,filter_total_count+100000) )
...
dds_adrenal <- dds[,dds$tissue == "Adrenal"]
dds_adrenal$tissue
rowSums(counts(dds_adrenal)) > 0 
...
calc_and_add_summarycounts_removezero <- function(summary, dds_1, dds_2){
  for ( tissue in levels(dds$tissue)){
    ...
    if (length(dds_tissue_1$id) > 1){
      dds_tissue_1[rowSums(counts(dds_tissue_1)) > 0]
      dds_tissue_2[rowSums(counts(dds_tissue_2)) > 0]
