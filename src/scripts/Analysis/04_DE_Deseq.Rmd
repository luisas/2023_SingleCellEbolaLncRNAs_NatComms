...
saveRDS(hurdle_summary_celltypes_lnc, paste0(robjectsdir,"celltypes_all.rds"))
...
#saveRDS(res_zinb_lncrnas, paste0(robjectsdir, "res_zinb_lncrnas.rds"))
...
#saveRDS(res_zinb_mrnas, paste0(robjectsdir, "res_zinb_mrnas.rds"))
...
#saveRDS(myeloids_live_24_subset_mrnasANDlncrnas, paste0(robjectsdir, "zinb_mrnasANDlncrnas.rds"))
...
myeloids_live_24_subset_mrnas <- subset(myeloids_live_24, features = annotated_mrnas)
sc <- as.SingleCellExperiment(myeloids_live_24_subset_mrnas)
filter <- rowSums(assay(sc)>0)>0
sc <- sc[filter, ]
filter <- colSums(assay(sc)>0)>0
sc <- sc[,filter]
