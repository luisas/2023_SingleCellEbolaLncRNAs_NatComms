...
cum_distr_perc <- plot_perc+stat_ecdf()+
          ...
          scale_color_brewer(palette = "Set1")+
          scale_fill_brewer(palette = "Set1")+
          labs(x = "%Cells expressing", y  = "Cumulative Density")+theme(legend.title = element_blank(), legend.position = "top")
...
scatter_plot_matching <- ggplot(matches_median, aes(x=medianexpr, y =perc_cells_expressing, col = as.factor(type)))+geom_point(size = 0.8)+labs( subtitle = paste0("# Matches ", n_matches))+ theme(legend.title = element_blank())+ geom_smooth(method = "loess")+scale_fill_brewer(palette = "Set1")+scale_color_brewer(palette = "Set1")+theme(legend.position = "none")+theme(panel.background = element_rect(fill = "white", colour = "grey50"))+labs(x = "Median Expression", y = "% Cells expressing")
...
scatter_plot_matching <- ggplot(matches_perc, aes(x=medianexpr, y =perc_cells_expressing, col = as.factor(type)))+geom_point(size = 0.8)+labs(title = " Matched % cells values", subtitle = paste0("# Matches ", n_matches))+ theme(legend.title = element_blank())+ geom_smooth(method = "loess")+scale_fill_brewer(palette = "Set1")+scale_color_brewer(palette = "Set1")+theme(legend.position = "none")+theme(panel.background = element_rect(fill = "white", colour = "grey50"))+labs(x = "Median Expression", y = "% Cells expressing")
...
matches
f1 <- FeaturePlot(immune.combined, features = c(genes), pt.size = 0.6, order = TRUE, cols = c("lightgrey", "blue"), max.cutoff = 3.5)
f1
```
...
VlnPlot(immune.combined, features = "ENSMMUG00000051358-unkown", pt.size = 0.01, slot = "data")
VlnPlot(immune.combined, features = "ADAMTSL4", pt.size = 0.01, slot = "data")
FeaturePlot(immune.combined, features = "ENSMMUG00000051358-unkown", slot = "data")

#df_taus_perc[df_taus_perc$Row.names =="CARD9",]
#qnExp["CD3D",]

#tauexp <- apply(qnExp, 1, function(test_gene) sum(1-(test_gene/max((test_gene))))/(length(test_gene)-1))
#tauexp <- as.data.frame(tauexp)
...
table(df_perc_tau$type)
df_perc_tau

#dim(df_perc_tau)
top <- df_perc_tau[df_perc_tau$tau ==1 ,]
top[order(top$n_cells),]
#hist(df_perc_tau[df_perc_tau$tau ==1,]$n_cells)
```
...
ggplot(df_perc_tau, aes(x = tau, col = type, fill = type ))+geom_density(alpha = 0.9)+theme_classic()

ggplot(df_perc_tau, aes(x = tau, col = type, fill = type ))+geom_density(alpha = 0.6)+theme_classic()+
...
ggplot(df_perc_tau, aes(x =n_cells, y = tau ))+geom_density()

# -----------------------------------------
# Percentage cells expressing vs tau 
# -----------------------------------------
p <- ggplot(df_complete_quartiles, aes(x = perc_cells_expressing, y = tau, color = type, fill = type))
p  +geom_point(alpha = 0.2, size = 0.2) + 
  geom_smooth(se = FALSE)+ scale_color_brewer(palette = "Set1")+ theme_classic()
...
cor(df_cor$tau, df_cor$perc_cells_expressing)
cor(df_cor$medianexpr, df_cor$perc_cells_expressing)
