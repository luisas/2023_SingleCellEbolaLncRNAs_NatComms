...
library(Seurat)
library(stringr)
data_dir <- "/home/luisas/Desktop/cluster/data1/01_Ebola-RNASeq/03_scRNA-Seq_test/01_digital_expression/"
...
get_Seurat_object <- function(file){
  ...
  pbmc <- CreateSeuratObject(pbmc.data, project = "10X_PBMC", min.cells = 3)
  return(pbmc)
}
...
list <- map(files[2:length(files)], get_Seurat_object)
pbmc <- get_Seurat_object(files[1])
list_names <- lapply(files, function(x) str_replace_all(unlist(rev(unlist(str_split(x, pattern= "/"))[-1])[[1]]), "_", "-"))

# -------------
#   Merge
# -------------
# Merge all seurats object in one 
pbmc.big <- merge(pbmc, y = list[2:length(list)], add.cell.ids = list_names, project = "PBMCbig")
