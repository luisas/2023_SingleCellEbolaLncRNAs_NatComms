...
# Dirs
dir_hisat_zyagen = file.path(datadir, "02_RNA-Seq_rheMac10/03_hisat")
```
...
dge <- dge_batch_blood
  ...
  colors <- c(brewer.pal(n = length(levels(dge$samples$complete_id)), name = 'Paired'),"black","blue")
  col.cell <- colors[dge$samples$complete_id]
  mds <- plotMDS(cpm(dge, log = TRUE,prior.count = 0.01), label= dge$samples$dpo, col=col.cell,main=" samples on genes CPM")
  #legend(5.5,3.2, inset=c(-0.2,0), legend=levels(dge$samples$tissue),  pch =c(16,17) )
  legend(5.5,2.9, inset=c(-0.2,0), legend=unique(dge$samples$complete_id),  fill= unique(colors[dge$samples$complete_id]), cex = 0.8)
# Filters out lowly expressed genes, calc CPM, plot
...
par(mar=c(10,8,4,2)) # increase y-axis margin.
colors <- brewer.pal(n = length(levels(dge$samples$tissue)), name = 'Paired')
col.cell <- colors[dge$samples$tissue]

density.group <- c(20,90)[dge$samples$group]

colnames(dge) <-  paste(dge$samples$tissue,dge$samples$dpo,dge$samples$id,  sep="_")
barplot(dge$samples$lib.size,names=colnames(dge),las=2,cex.names=0.7,  col = col.cell)
...
input <- as.list(as.data.frame(assays(dds_batch_filter)$logCPM))
names_input_legend <-unlist(lapply( names(input), function(a) paste(strsplit(a,"/")[[1]][11], strsplit(a,"/")[[1]][12],strsplit(a,"/")[[1]][13] )))
unique(names_input_legend)
col <- c(brewer.pal(n = length(names_input_legend), name = 'Paired'),"black","blue")
par(mfrow = c(1, 1), mar = c(4, 5, 1, 1))
multidensity(input, xlab = "log2 CPM",col = col , legend = list(
x = "right",
legend = names_input_legend, fill = col, cex=0.8),
             main = "Batch_all", cex.axis = 1.2, cex.lab = 1.5, las = 1)
...
RA1834 <- dds_batch[,dds_batch$id == "RA1834"]
par(mfrow = c(1, 1), mar = c(4, 5, 1, 1))
multidensity(as.list(as.data.frame(assays(RA1834)$logCPM)), xlab = "log2 CPM", legend = NULL,
             main = "RA1834", cex.axis = 1.2, cex.lab = 1.5, las = 1)
...
RA1834 <- dds_batch_filter[,dds_batch_filter$id == "RA1834"]
par(mfrow = c(1, 1), mar = c(4, 5, 1, 1))
multidensity(as.list(as.data.frame(assays(RA1834)$logCPM)), xlab = "log2 CPM", legend = NULL,
             main = "RA1834-filter", cex.axis = 1.2, cex.lab = 1.5, las = 1)
```
