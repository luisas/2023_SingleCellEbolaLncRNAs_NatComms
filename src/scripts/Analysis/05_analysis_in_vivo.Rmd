...
We see two peaks, the firt one of which corresponds to lowly expressed genes. As explained in [Luecken MD et al.](https://www.embopress.org/doi/pdf/1FDRTHRESHOLD5252/msb.20188746): "a guideline to setting this threshold is to use the minimum cell cluster size that is of interest and leaving some leeway for dropout effects". As we will not rely on clusters that have fewer than 15 cells, we will use it as a filter:
...
p1 <- DimPlot(immune.combined, reduction = "umap", group.by = "dpi", pt.size = FDRTHRESHOLD, cols = brewer.pal(10, "Paired"))+theme_minimal()+ theme(panel.background = element_rect(fill = "white", colour = "grey50"), panel.grid.major = element_blank(), panel.grid.minor = element_blank())+theme(legend.text = element_text(size=18))+theme(legend.text = element_text(size=20), axis.title = element_text(size = 15))
p2 <- DimPlot(immune.combined, reduction = "umap", group.by = "individual")
p3 <- DimPlot(immune.combined, reduction = "umap", label = TRUE)

immune.combined$batch <- ifelse(immune.combined$batch =="fresh", "Fresh", "Frozen")
p4 <- DimPlot(immune.combined, reduction = "umap", group.by = "batch", pt.size = 0.3, cols = brewer.pal(3, "Set1")) +theme_minimal()+ theme(panel.background = element_rect(fill = "white", colour = "grey50"), panel.grid.major = element_blank(), panel.grid.minor = element_blank())+theme(legend.text = element_text(size=20), axis.title = element_text(size = 15))
p4
...
DimPlot(immune.combined, reduction = "umap", label = TRUE, cols =brewer.pal(3, "Set2"),pt.size = 0.3, label.size = 6)+theme_minimal()+ theme(panel.background = element_rect(fill = "white", colour = "grey50"), panel.grid.major = element_blank(), panel.grid.minor = element_blank())+theme(legend.text = element_text(size=18))+theme(legend.text = element_text(size=20), axis.title = element_text(size = 15))
...
get_de_names <- function(df_filtered, FDRTHRESHOLD = FDRTHRESHOLD, FCTHRESHOLD = log(1), subset = all_lncrnas){
  # Only select lncrnas 
...
z_score_heatmap <- function(subset, celltype,de_lnc_all){
  # Exprssion matrix
  ...
  column_ha <- HeatmapAnnotation(celltype = unlist(lapply( colnames(z_score_mean_matrix), function(x) unlist(str_split(x, "_"))[1])), 
                          comparison = unlist(lapply( colnames(z_score_mean_matrix), function(x) unlist(str_split(x, "_"))[2])),
                          show_annotation_name = FALSE, 
                          col = list(celltype = c("myeloid" = brewer.pal(3, "Set2")[3],
                                                  "T" = brewer.pal(3, "Set2")[1],
                                                  "B" = brewer.pal(3, "Set2")[2]),
  ...
  h <- Heatmap(z_score_mean_matrix,name = "Z-Score", show_row_dend = FALSE,
            show_column_dend = FALSE,
            cluster_columns = FALSE,
            cluster_rows = TRUE,
            cluster_row_slices = FALSE,
            ...
            row_title_rot = 0,
            top_annotation = column_ha, 
            column_split = gsub("myeloid", "Myeloid",unlist(lapply( colnames(z_score_mean_matrix), function(x) unlist(str_split(x, "_"))[1]))))
  
  # Add legend
  leg_comparison <- Legend(at = c("Baseline", "Early", "Middle", "Late"), title = "Stage", legend_gp = gpar(fill = brewer.pal(8,"Pastel2")[5:9]))
  hm_wlegend <- draw(h,annotation_legend_list = c(leg_comparison))
  return(hm_wlegend)
}

z_score_heatmap(Myeloids, "myeloid", de_lists_lnc_my)
z_score_heatmap(T, "T", de_lists_lnc_T)
z_score_heatmap(B, "B", de_lists_lnc_B)
...
venn.diagram(
  ...
  filename = '/home/luisas/Desktop/venn_diagramm_celltype',
  output = TRUE ,
          imagetype="png" ,
          height = 580 , 
          width = 580 , 
...
venn.diagram(
  ...
  filename = '/home/luisas/Desktop/venn_diagram_stagessignature_de',
  output = TRUE ,
          imagetype="png" ,
          height = 680 , 
          width = 680 , 
          ...
          fill = c(alpha(brewer.pal(3, "Set1")[1],0.1), alpha(brewer.pal(3, "Set1")[2],0.1), alpha(brewer.pal(3, "Set1")[3],0.1)),
          cex = 0.8,
          fontfamily = "times",
          cat.cex = 0.7,
          cat.default.pos = "outer",
...
plot_seperate_features(immune.combined, gene  = c("ENSMMUG00000061948-unkown"), ident = "group", pt.size = 1, colors =c('#DCD9DE','navy'))
plot_seperate_features(immune.combined, gene  = c("PVRL1"), ident = "group", pt.size = 1)
plot_seperate_features(immune.combined, gene  = c("THY1"), ident = "group", pt.size = 1)
...
lnc_ranges_complete_de <- lnc_ranges_complete[lnc_ranges_complete$gene_id %in% gsub("-", "_",gsub("-unkown", "","ENSMMUG00000061948-unkown")), ]
...
logfc <- stats_complete[stats_complete$primerid %in% unlist(unique(all_lnc_1mb)),]$coef
 ego <- clusterProfiler::enrichGO(gene = unlist(unique(all_lnc_1mb)),OrgDb =org.Hs.eg.db, keyType = keytype,ont = "BP",universe =unique(ref$gene_name))
...
significant_correlations <- correlations[pvals < FDRTHRESHOLD & rhos !=1]
significant_pairs <- pairs_list[pvals < FDRTHRESHOLD & rhos !=1]
significant_correlations

list <- genes
# Extract mrnas from significant correlations 
do_go <- function(list, keytype = "SYMBOL"){
  ego <- clusterProfiler::enrichGO(gene = list,OrgDb =org.Hs.eg.db, keyType = keytype,ont = "BP",universe =unique(ref$gene_name))
