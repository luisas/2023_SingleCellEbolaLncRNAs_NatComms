...
```{r results='hide', message=FALSE, warning=FALSE}
gr <- novel_lncrnas_ranges
export(gr, "/home/luisas/Desktop/explorative/gr.gtf")
ho_unlisted <- unlist(Human_Orthologs)
df <- data.frame("gene_id" = gr$gene_id,"transcript_id" = gr$transcript_id,"exon_number" = gr$exon_number,  "range_width" = width(ranges(gr)))
#  LiftOver  
...
gr_toHuman_collapsed[[13]]
overlaps1 <- lapply(gr_toHuman_collapsed[[13]], function(x) findOverlaps(x,human_annotation,minoverlap=1L))
overlaps1
hits_in_human1 <-lapply(overlaps, function(x) human_annotation[subjectHits(x)])
hits_in_human1
length(hits_in_human1)
orthologs_class1 <- (lapply(hits_in_human, function(x) (unique(x$gene_biotype))))
orthologs_class1

export(gr_toHuman_collapsed[[13]][[1]], "/home/luisas/Desktop/explorative/test2_human.gtf")
...
length(orthologs_class) - length(orthologs_class2)
length(orthologs_class[mask])
orthologs_class <- orthologs_class[mask]

orthologs_class2 <- unlist(orthologs_class)
length(orthologs_class2)
save(orthologs_class,file="orthologsclass.Rda")
...
png("/home/luisas/Desktop/explorative/myplot.png")
myplot <- ggplot(data.frame(orthologs_class2), aes(x=orthologs_class2, fill = orthologs_class2)) +
