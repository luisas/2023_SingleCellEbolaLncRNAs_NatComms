...
candidates_lncRNA <- import(file.path(datadir, "02_RNA-Seq_rheMac10/05_lncrnaAnnotation/feelnc_gencode_linc/candidate_lncRNA.gtf"))
lncRNAs_out <- import(file.path(datadir, "02_RNA-Seq_rheMac10/05_lncrnaAnnotation/feelnc_gencode_linc/feelnc_codpot_out/candidate_lncRNA.gtf.lncRNA.gtf"))
mRNAs_out <- import(file.path(datadir, "02_RNA-Seq_rheMac10/05_lncrnaAnnotation/feelnc_gencode_linc/feelnc_codpot_out/candidate_lncRNA.gtf.mRNA.gtf"))
...
I want to select from the novel ones only the ones: 
- Novel
- Expressed at least in 3 samples
- Intergenic

Novels - is it ok with the labels I am using? 
...
# The missed ones
intersect_missed_mrnaout

table(strand(lncRNAs_ref[lncRNAs_ref$gene_id %in% intersect_missed_mrnaout]))
...
histo_length_density <- function(gr,title,color){
  df <- data.frame("gene_id" = gr$gene_id,"transcript_id" = gr$transcript_id, "range_width" = width(ranges(gr)))
  collapsed <- df%>% dplyr::group_by(gene_id,transcript_id) %>% dplyr::summarize("range" = sum(range_width))
  p1 <- ggplot(collapsed, aes(x=range)) +
        geom_density(alpha=0.5, fill = color, color = "Darkgrey")+
        labs(title=title) + ylab("Density") + xlab("Transcript length")+
        theme(plot.title = element_text(hjust = 0.5))+
        xlim(0,10000)+
...
p1 <-histo_length_density(novel_lncrnas_ranges," NOVEL",colors[2]) 
p2 <-histo_length_density(lncrnas_ref_collapsed," REF MACAQUE",colors[3])
p4 <-histo_length_density(lncrnas_ref_collapsed_human,"REF HUMAN","red")
ggarrange( p1,p2,p4,  ncol=1, nrow=3, top = "Transcripts Lengths",  legend="top")
p1 + scale_fill_discrete(breaks=c("trt1","ctrl","trt2"))
# --------------------------------------------------
...
```{r}
#-----------------------------
# Get the exon levels of the highly expressed ones

dds_zyagen <- DESeqDataSetFromMatrix(countData = countData, colData = colData, design = ~tissue)
dge_zyagen <- DGEList(counts = assays(dds_zyagen)$counts, group = dds_zyagen$tissue, genes = rownames(dds_zyagen))
assays(dds_zyagen)$logCPM <- cpm(dge_zyagen, log = TRUE,prior.count = 0.5)
mask <- rowMeans(assays(dds_zyagen)$logCPM) > 0
expressed_ones <- dge_zyagen[mask, ]

assays(dds_zyagen)$counts

table(novel_lncrnas_ranges_collapsed$gene_id %in% expressed_ones$genes$genes)
#FALSE  TRUE 
#21096 15339 
novel_lncrnas_ranges_collapsed_expressed <- novel_lncrnas_ranges_collapsed[novel_lncrnas_ranges_collapsed$gene_id %in% expressed_ones$genes$genes ,]

length(novel_lncrnas_ranges_collapsed)

colors <- c("red","#D16103","#4E84C4")
p3 <- plot_length_exons(lncrnas_ref_collapsed,novel_lncrnas_ranges_collapsed_expressed,lncrnas_ref_collapsed_human,colors)
p1 <-histo_length_exon(novel_lncrnas_ranges_collapsed_expressed," NOVEL",colors[2])
p2 <-histo_length_exon(lncrnas_ref_collapsed," REF MACAQUE",colors[3])
p4 <-histo_length_exon(lncrnas_ref_collapsed_human,"REF HUMAN","red")
g1 <- as.grob(p1); g2 <- as.grob(p2) ; g3 <- as.grob(p3) ; g4 <- as.grob(p4)
grid.arrange( grobs = list(g1,g2,g3,g4),layout_matrix = rbind(c(1,2,4),c(3,3,3)), top = "Exon Lengths for higlhy expressed ones")
```
...
##  Orthologs
```{r}
#chainfile <- file.path(projdir,"/data/01_PreliminaryFiles_rheMac8/gene_annotations/chains/hg19ToHg18.over.chain")
...
df <- data.frame("gene_id" = gr$gene_id,"transcript_id" = gr$transcript_id,"exon_number" = gr$exon_number,  "range_width" = width(ranges(gr)))
collapsed <- df%>% dplyr::group_by(gene_id,transcript_id, exon_number) %>% dplyr::summarize("range" = sum(range_width))
long_exons <- tail(collapsed[order(collapsed$range),], n = 10)$gene_id
a <- novel_lncrnas_ranges[novel_lncrnas_ranges$transcript_id == "MSTRG.32406.1",]
ho <- unlist(Human_Orthologs)
query <- ho[ho$transcript_id == "MSTRG.32406.1",]
# The selected gene is 	MSTRG.32406.1
gr <- (liftOver(a, chain))
query_human_ortholog_region <- range(split(gr[[1]], ~exon_number))
overlaps <- findOverlaps(query_human_ortholog_region,human_annotation)
hits_in_human <- human_annotation[subjectHits(overlaps)]
hits_in_human$gene_biotype
  hits_in_human
export(hits_in_human, "/home/luisas/Desktop/explorative/MSTRG.32406.1_human.gtf")
```

Now I want to apply that rule to all of them and see what comes out. 
The question is: from the novel identified one, see their orthologs and find some stats.
```{r}
gr_macaque <- novel_lncrnas_ranges

# "Translate the coordinates to human"
gr_toHuman <- liftOver(gr_macaque, chain) 
gr_toHuman_chr3 <- gr_toHuman
...
# I want to find some that are putative lncRNAs of quality!


Investigating the rnas predicted as mRNAs, why? 
- Length 
