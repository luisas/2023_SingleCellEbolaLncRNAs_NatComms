...
df_celltype <- readRDS("/home/luisas/Desktop/cluster/data/02_scRNA-Seq_PBMCs/01_scRNA-Seq_inVivo_rhemac10/05_RObjects/05_stats/df_mrna_celltype_permutation.rds")
...
table(df_celltype$gene_id %in% annotated_mrnas)
...
spec_scores <- lapply(as.character(unique((df_celltype$gene_id))[1:2]), function(gene) calc_score_gene(df_celltype, gene))

gene <- as.character(unique((df_celltype$gene_id))[1])
calc_score_gene(df_celltype, gene)
