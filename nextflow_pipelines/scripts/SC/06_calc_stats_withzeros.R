...
calc_mean_and_percentage_cell <- function(subset,ident = "", df, threshold = 1 ){
  ...
  meanexpr <- rowMeans(expression_matrix, na.rm = FALSE)
  medianexpr <- rowMedians(expression_matrix, na.rm = FALSE)
  maxexpr <- rowMaxs(expression_matrix, na.rm = FALSE)
  minexpr <- rowMins(expression_matrix, na.rm = FALSE)
  var <- rowVars(expression_matrix, na.rm = FALSE)
...
# Save 
saveRDS(df_lnc, file.path(robjectsdir, "df_lnc_withzeros.rds"))
saveRDS(df_mrna, file.path(robjectsdir, "df_mrna_withzeros.rds"))
...
# Save
saveRDS(df_lnc_celltype, file.path(robjectsdir, "df_celltype_lnc_withzeros.rds"))
saveRDS(df_mrna_celltype, file.path(robjectsdir, "df_mrna_celltype_withzeros.rds"))
