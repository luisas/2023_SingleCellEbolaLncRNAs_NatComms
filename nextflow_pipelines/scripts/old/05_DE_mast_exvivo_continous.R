...
#file =  "/home/luisas/Desktop/cluster/data/02_scRNA-Seq_PBMCs/00_scRNA-Seq_exVivo_rhemac10/05_RObjects/06_correlation/myeloids.rds"
...
get_mast_model  <- function(subset, contarst = "viral_load"){
  ...
  colData(subset)$viral_load<-subset$viral_load
  
  # Set Up the model 
  #zlmCond <- zlm(~condition + cngeneson , scam)
  zlmCond <- zlm(~viral_load + cngeneson , scam)
...
# --------------------------------------
mast_model <- get_mast_model(myeloids, contarst = "viral_load")
saveRDS(mast_model,file.path(robjectsdir, paste0("MAST_exvivo_model_viralload_myeloids.rds")))


# Only on live Myeloids
myeloid_live <- myeloids[,myeloids$cond == "live"  ]
mast_model <- get_mast_model(myeloid_live, contarst = "viral_load")
saveRDS(mast_model,file.path(robjectsdir, paste0("MAST_exvivo_model_viralload_myeloids_LIVE.rds")))

# Only on live Myeloids 24h 
myeloid_live_24 <- myeloid_live[,myeloid_live$hpi == "H024"  ]
mast_model <- get_mast_model(myeloid_live_24,contarst = "viral_load")
saveRDS(mast_model,file.path(robjectsdir, paste0("MAST_exvivo_model_viralload_myeloids_LIVE_24only.rds")))
