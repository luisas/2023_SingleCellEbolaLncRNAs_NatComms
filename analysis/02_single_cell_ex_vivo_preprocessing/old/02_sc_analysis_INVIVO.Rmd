...
saveRDS(pbmc.big, file.path(result_path,"INVIVO_rhemac10_merged.rds"))
```
##---------------------------------------------------------
##                           Cell QC 
...
pbmc.big <- readRDS(file.path(result_path,"INVIVO_rhemac10_merged.rds"))
...
saveRDS(pbmc,file.path(result_path,"INVIVO_rhemac10_aftercellQC.rds"))
```
...
```{r}
pbmc <- readRDS(file.path(result_path,"INVIVO_rhemac10_aftercellQC.rds"))
...
#pbmc <- readRDS(file.path(result_path,"INVIVO_rhemac10_aftercellQC_aftercellandgeneqc.rds"))
...
writeMM(obj = sparse.gbm, file=file.path(result_path,"INVIVO_sparse.mtx"))
```

# === CLUSTER ===  --> Run Scrublet in Python ( see nextflow_pipelines/01_CLUSTER_MARENOSTRUM)

```{r cars}
cell_mask_scrublet <- read.table(file.path(result_path, "INVIVO_predicted_doublet_mask.mtx"))
doublet_scores_scrublet <- read.table(file.path(result_path, "INVIVO_predicted_doublet_scores.txt"))

list <- as.list(as.character(cell_mask_scrublet$V1))
mask <- list == "True"
new <- pbmc[,!mask]

saveRDS(new, file.path(result_path,"INVIVO_rhemac10_aftercellandgeneQC_afterScrublet.rds"))
```
...
```{r cars}
#immune.combined <- readRDS(file.path(result_path,"INVIVO_rhemac10_aftercellandgeneQC_afterScrublet.rds"))
# Normalize 
...
saveRDS(immune.combined, file.path(result_path, "INVIVO_preprocessed_ready.rds"))

# ----------------------------
# FIND MARKERS  for every cluster compared to all remaining cells, report only the positive ones
pbmc.markers <- FindAllMarkers(immune.combined, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
saveRDS(pbmc.markers, file.path(result_path,"INVIVO_markers_rhemac10_2k.rds"))
