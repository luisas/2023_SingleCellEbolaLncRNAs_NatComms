...
```{r cars}
#immune.combined <- readRDS(file.path(data_path,"/05_RObjects/03_prep/03_immune.combined_viral.load.rds"))
ebola_genes <- rownames(immune.combined)[rownames(immune.combined) %in% paste(ebola_ref$gene_id, "unknown", sep ="-")]
ebola_genes
FeaturePlot(immune.combined, features  = cellsubset)
plot_genes(immune.combined, cellsubset,0.5, col = "#DA0202", "Cells expressing Ebola RNA", sizes.highlight = 0.4)
```
...
saveRDS(all_lncrnas, file.path(robjectsdir, "all_lncrnas_all.filt.rds"))
saveRDS(annotated_mrnas, file.path(robjectsdir, "annotated_mrnas_all.filt.rds"))
```