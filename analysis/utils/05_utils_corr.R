...
# Methods
filter_significant_correlations <- function(correlations_exvivo, ref, all_lncrnas, annotated_mrnas, FDRTHRESHOLD, RHOTHRESHOLD){
...
plot_corr_genes_stat <- function(Approach_C_significant_exvivo, filename){
  ...
  pdf(file.path(plots, filename), width = 5, height = 6)
  p
  dev.off()
  p
}