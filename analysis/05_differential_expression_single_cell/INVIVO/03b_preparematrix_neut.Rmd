...
# --------------------------------------------
do_go_community<- function(list, universe,n,de_lnc_names){
...
community_check <- function(n){ do_go_community(lou2$names[lou2$membership == n],universe_myeloids,n, de_lnc_mono) }
...
genes <-c(mamu_dr, pos_2_pc, neat1_gene)
...
Dotplot_mod(monocyte, features = c(one_c_hladr_LNC), group.by = "group", dot.scale =10,scale.by = "radius", cols = c( "lightblue", "darkred"))+coord_flip()+theme_paper+xlab("")+ylab("")+theme_bw()+ggtitle("Monocytes")+theme_test()+theme(axis.text.x= element_text(size = 15),axis.text.y= element_text(size = 12))+theme(title = element_text(size = 15))
