...
de_lnc_all <- c(de_lists_lnc_my, de_lists_lnc_T, de_lists_lnc_B,de_lists_lnc_neut)
...
de_mrna_all <- c(de_lists_mrna_my, de_lists_mrna_T, de_lists_mrna_B,de_lists_mrna_neu)
...
saveRDS(de_all, "/home/luisas/Desktop/cluster/data/02_scRNA-Seq_PBMCs/01_scRNA-Seq_inVivo_rhemac10/05_RObjects/04_DE_neut//de_all_stages.rds")
saveRDS(unique(unlist(de_lists_lnc)), "/home/luisas/Desktop/cluster/data/02_scRNA-Seq_PBMCs/01_scRNA-Seq_inVivo_rhemac10/05_RObjects/04_DE_neut//de_lnc.rds")
...
DimPlot(immune.combined, split.by = "group", cols = c(brewer.pal(4, "Set2")))+theme_sc
DimPlot(neut, group.by = "group", cols = rev(c(brewer.pal(4, "Set1"))))+ggtitle("Neutrophils colored by infection stage")
```
...
df_de$y_pos <- c(11+10,27+20,5+10,10,6,6)
...
saveRDS(stats_complete, "/home/luisas/Desktop/cluster/data/02_scRNA-Seq_PBMCs/01_scRNA-Seq_inVivo_rhemac10/05_RObjects/04_DE_neut//stats_complete.rds")
saveRDS(m, "/home/luisas/Desktop/cluster/data/02_scRNA-Seq_PBMCs/01_scRNA-Seq_inVivo_rhemac10/05_RObjects/04_DE_neut//m.rds")
...
# Alternative visualization instead of HeatMap
```{r plotData}
candid <- unique(lnc_de_all$primerid)
plot.data <- Dotplot_data(immune.combined, features = candid, group.by = "group",scale.by = "radius", cols = c( "lightblue", "darkred"))
plot.data$name <- unlist(lapply(plot.data$features.plot, get_orthologname))
plot.data$id <- factor(plot.data$id, levels = c("baseline", "early", "middle", "late"))
plot.data$labels <- (plot.data$id)
...
FC_heatmap_subset_celltype(m, "T", unique(unlist(de_lists_lnc_T)), "T",  brewer.pal(3, "Set2")[1],orthologs, lncpedia, reported_immlnc, close_isg_summary, unique(close_broad), legend = T)
dev.off()
```
```{r heatmapBlnc}
png(filename = "/home/luisas/Desktop/cluster/data/plots/neut/B_lnc_FC.png", width = 1300, height = 1100)
FC_heatmap_subset_celltype(m, "B", unique(unlist(de_lists_lnc_B)), "B",  brewer.pal(3, "Set2")[1],orthologs, lncpedia, reported_immlnc, close_isg_summary, unique(close_broad), legend = T )
dev.off()
```
```{r heatmapMonolnc}
png(filename = "/home/luisas/Desktop/cluster/data/plots/neut/Mono_lnc_FC.png", width = 1300, height = 1100)
FC_heatmap_subset_celltype(m, "Monocyte", unique(unlist(de_lists_lnc_my)), "Monocyte",  brewer.pal(3, "Set2")[1],orthologs, lncpedia, reported_immlnc, close_isg_summary, unique(close_broad), legend = T )
dev.off()
```

```{r heatmapMonolncExpr}
png(filename = "/home/luisas/Desktop/cluster/data/plots/neut/Mono_lnc_expr.png", width = 1300, height = 1100)
m_expr <- acast(plot.data, features.plot~labels, value.var="avg.exp.scaled")
colnames(m_expr) <- paste("Monocyte", colnames(m_expr), sep = "_")
FC_heatmap_subset_celltype(m_expr, "Monocyte", unique(unlist(de_lists_lnc_my)), "Monocyte",brewer.pal(3, "Set2")[3],orthologs, lncpedia, reported_immlnc,close_isg_summary, unique(close_broad), legend = T )
dev.off()
```
```{r heatmapTlncExpr}
png(filename = "/home/luisas/Desktop/cluster/data/plots/neut/T_lnc_expr.png", width = 1300, height = 1100)
m_expr <- acast(plot.data, features.plot~labels, value.var="avg.exp.scaled")
colnames(m_expr) <- paste("T", colnames(m_expr), sep = "_")
FC_heatmap_subset_celltype(m_expr, "T", unique(unlist(de_lists_lnc_T)), "T",  brewer.pal(3, "Set2")[1],orthologs, lncpedia, reported_immlnc,close_isg_summary, unique(close_broad), legend =T ) 
dev.off()
```
```{r heatmapBlncExpr}
png(filename = "/home/luisas/Desktop/cluster/data/plots/neut/B_lnc_expr.png", width = 1000, height = 1100)
m_expr <- acast(plot.data, features.plot~labels, value.var="avg.exp.scaled")
colnames(m_expr) <- paste("B", colnames(m_expr), sep = "_")
FC_heatmap_subset_celltype(m_expr, "B", unique(unlist(de_lists_lnc_T)), "B",  brewer.pal(3, "Set2")[1],orthologs, lncpedia, reported_immlnc,close_isg_summary, unique(close_broad), legend = T )
dev.off()
```

```{r heatmapMonopc}
png(filename = "/home/luisas/Desktop/cluster/data/Mono_pc_FC.png", width = 1300, height = 1100)
FC_heatmap_subset_celltype(m, "Myeloid", unique(unlist(de_lists_mrna_my)), "Myeloid",brewer.pal(3, "Set2")[3],orthologs, lncpedia, reported_immlnc, close_isg_summary, unique(close_broad))
dev.off()
```
```{r heatmapTpc}
png(filename = "/home/luisas/Desktop/cluster/data/plots/T_pc_FC.png", width = 1300, height = 1100)
FC_heatmap_subset_celltype(m, "T", unique(unlist(de_lists_mrna_T)), "T",  brewer.pal(3, "Set2")[1],orthologs, lncpedia, reported_immlnc, close_isg_summary, unique(close_broad))
dev.off()
```
```{r heatmapBpc}
...
# Subset to one celltype only 
#distances <- distances[distances$lnc %in% gsub("-unknown", "", lnc_T_all$primerid),]
...
# Use modules instead of genemanes
lnc_de_all$dir <- ifelse(lnc_de_all$coef >0 , "up", "down")
lnc_de_all$comp_more <- paste(lnc_de_all$comparison,lnc_de_all$dir, sep = "-" )

comparisons_lnc <- lnc_de_all[gsub("-unknown", "", lnc_de_all$primerid) %in% distances$lnc,c("primerid", "dir", "comparison")]
comparisons_lnc_summary <- comparisons_lnc %>% dplyr::group_by(primerid) %>% dplyr::mutate(comparison_p = paste0(unique(dir), collapse = ","))
...
distances$comp_our <- comparisons_lnc_summary[distances$lnc,]$comparison_p

comparisons_lnc_summary <- comparisons_lnc %>% dplyr::group_by(primerid) %>% dplyr::mutate(comparison_p = paste0(unique(comparison), collapse = ","))
comparisons_lnc_summary <- as.data.frame(distinct(comparisons_lnc_summary[,c("primerid", "comparison_p")]))
rownames(comparisons_lnc_summary) <- gsub("-unknown", "", comparisons_lnc_summary$primerid)
...
table(broad_dist[order(broad_dist$d),]$BROAD_module)

library(tidyverse)
# restrict to mono and global 
#broad_dist <- broad_dist[grepl("T|Global", broad_dist$BROAD_module),]
spread_dist <- broad_dist[!is.na(broad_dist$BROAD_module), ] %>% 
  # create an auxiliary variable
  mutate(AUX = 1) %>%
  # spread the data from long to wide and fill the empty cells with 0
  spread(BROAD_module, AUX, fill = 0)
spread_dist <- (spread_dist[,c(7,seq(9,ncol(spread_dist)))] )
spread_dist <- dplyr::group_by(spread_dist, comp_our) %>% dplyr::summarise_all(sum)
#m<-as.matrix(spread_dist[,-1])
#rownames(m)<-spread_dist[,1]$comp_our
...
```{r colocation}
png(filename = "/home/luisas/Desktop/cluster/data/plots/colocation_T.png", width = 700, height = 700)
grid.col = c(up = "red", down = "blue")
chordDiagram(m, transparency = 0.5, grid.col = grid.col)
dev.off()
```
...
names(x)  <-  c( "T DE lncRNAs " , "B DE lncRNAs ","Myeloid DE lncRNAs" )
upset(fromList(x), order.by = "freq",text.scale = c(2.5, 2.5, 2.5, 1.0, 2, 2.15))

intersect(intersect(x$`T DE lncRNAs `, x$`B DE lncRNAs `), x$`Myeloid DE lncRNAs`)

# Stages 
x_stages <- list( unlist(unique(early_de_lnc)), unlist(unique(middle_de_lnc)),unlist(unique(late_de_lnc)))
names(x_stages)  <-  c( "T DE mRNAs " , "B DE mRNAs ","Myeloid DE mRNAs" )
upset(fromList(x_stages), order.by = "freq")
```
...
venn.diagram(
  ...
  filename = '/home/luisas/Desktop/cluster/data/plots/vennsneut/venn_diagramm_celltype_lnc',
  output = TRUE ,
          imagetype="png" ,
          height = 580 , 
          width = 580 , 
...
venn.diagram(
  ...
  filename = '/home/luisas/Desktop/cluster/data/plots/vennsneut/venn_diagram_stagessignature_de_lnc',
  output = TRUE ,
          imagetype="png" ,
          height = 680 , 
          width = 680 , 
...
venn.diagram(
  ...
  filename = '/home/luisas/Desktop/cluster/data/plots/vennsneut/venn_diagramm_celltype_pc',
  output = TRUE ,
          imagetype="png" ,
          height = 580 , 
          width = 580 , 
