...
# Gene annotation 
ref <- import(file.path("/home/luisas/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation/03_novel_lncRNAs_list/rheMac10_EBOV_and_novel_genenames.gtf1"))
...
```{r a}
# Complete list of differentially expressed lncRNAs

de_all <- unique(unlist(de_lists_lnc))
m <- mbackup
# Create Matrix 
m_significance <- msig_backup 
m <- m[rownames(m) %in% de_all,, drop = FALSE]
m_significance<- m_significance[rownames(m_significance) %in% de_all,, drop = FALSE]
rownames(m) <- gsub("-unkown","",rownames(m))
column_ha <- HeatmapAnnotation(celltype = unlist(lapply( colnames(m), function(x) unlist(str_split(x, "_"))[1])), comparison = unlist(lapply( colnames(m), function(x) unlist(str_split(x, "_"))[2])),
show_annotation_name = FALSE,
col = list(celltype = c("T" = brewer.pal(3, "Set2")[1],
                                   "B" = brewer.pal(3, "Set2")[2],
                                   "myeloid" = brewer.pal(3, "Set2")[3]),
           comparison =c("early" = brewer.pal(6, "Set2")[4],
                                   "middle" = brewer.pal(6, "Set2")[5],
                                   "late" = brewer.pal(6, "Set2")[6]) ))
dim(m)
dim(m_significance)

rowlabels <- replace(substring(rownames(m), 1,4),substring(rownames(m), 1,4) %in% substr(ebola_ref$gene_id, 1,4), NA)
row_ha_type <- rowAnnotation(annot =rowlabels,
                        show_annotation_name = FALSE, 
                        col = list(annot = c("ENSM" = brewer.pal(3, "Paired")[1],
                                   "poly" = brewer.pal(3, "Paired")[2],
                                   "ribo" = brewer.pal(3, "Paired")[3])))

labels_bulk <- ifelse(gsub("-", "_",gsub("-unkown", "", rownames(m))) %in% bulk_de_genes_all, "Also DE in Bulk", "na")
row_ha <- rowAnnotation( genetype =labels_bulk,
                        show_annotation_name = FALSE,
                        col = list(genetype = c("na" = "white", "Also DE in Bulk" = "orange")))
Heatmap(m, show_row_dend = FALSE,
        ...
        left_annotation = row_ha_type,
          column_split = unlist(lapply( colnames(m), function(x) unlist(str_split(x, "_"))[1])),
          cell_fun = function(j, i, x, y, width, height, fill) {
            if(m_significance[i, j] < 0.1 & abs(m[i,j]) > log(1.2) )
...
de_lists_lnc_my <- list(lnc_myeloid_late)
names(de_lists_lnc_my) <- c("lnc_myeloid_late")
...
seurat_object_myeloid_late <- subset(immune.combined, features= unique(unlist(de_lists_lnc_my)))
