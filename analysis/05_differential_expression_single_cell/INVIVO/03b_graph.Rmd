...
```{r colocation}
png(filename = "/home/luisas/Desktop/graph.png", width = 2300, height = 2100)
plot(lou2, gD2,  vertex.size=2, edge.arrow.size = .1, col=V(gD2)$color, layout=layout.fruchterman.reingold(gD2, niter=10000), edge.color= "#7A7A7A", vertex.frame.width = 8, mark.col=c(brewer.pal(11 , "Set3"), alpha.f = .3),  mark.border=brewer.pal(11 , "Set3"),vertex.label = ifelse(names(as.list(V(gD2))) %in% de_lnc_mono, names(as.list(V(gD2))), NA))
dev.off()
```
...
plot(lou2, gD2,
    ...
    vertex.label.color="black", vertex.label.font=1.9,
    vertex.label.cex=2.9,
...
# Visulize community 
n <- 7
gn <- subgraph(gD2, V(gD2)[community==n])
V(gn)$color <- ifelse(names(as.list(V(gn))) %in% gsub("-unknown", "",de_lnc_names), "red", "grey")
plot(gn,vertex.size=7, edge.arrow.size = .1, col=V(gn)$color, layout=layout.fruchterman.reingold(gn, niter=10000), edge.color= "#7A7A7A", vertex.frame.width = 8, mark.col=c(brewer.pal(11 , "Set3"), alpha.f = .3),  mark.border=brewer.pal(11 , "Set3"),vertex.label = names(as.list(V(gn))),    vertex.label.color="black", vertex.label.font=1.9,
    vertex.label.cex=0.9,
    vertex.label.dist=0.1 )


plot_region("MSTRG.41819-unknown", range = 100000)


```

# ---------------------------------------------------------------------
#                         Approach number 2 (Not ok)
