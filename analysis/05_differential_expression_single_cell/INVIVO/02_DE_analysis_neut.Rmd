...
de_lists_lnc <- list(lnc_Monocyte_late$primerid,lnc_Monocyte_middle$primerid, lnc_Monocyte_early$primerid,lnc_T_late$primerid, lnc_T_middle$primerid, lnc_T_early$primerid, lnc_B_late$primerid, lnc_B_middle$primerid, lnc_B_early$primerid,  lnc_Neutrophil_late$primerid, lnc_Neutrophil_middle$primerid, lnc_Neutrophil_early$primerid )
names(de_lists_lnc) <- c("lnc_Monocyte_late", "lnc_Monocyte_middle", "lnc_Monocyte_early","lnc_T_late", "lnc_T_middle", "lnc_T_early", "lnc_B_late", "lnc_B_middle", "lnc_B_early", "lnc_Neutrophil_late", "lnc_Neutrophil_middle", "lnc_Neutrophil_early")


de_lists_mrna <- list(mrna_Monocyte_late$primerid,mrna_Monocyte_middle$primerid, mrna_Monocyte_early$primerid,mrna_T_late$primerid, mrna_T_middle$primerid, mrna_T_early$primerid, mrna_B_late$primerid, mrna_B_middle$primerid, mrna_B_early$primerid,  mrna_Neutrophil_late$primerid, mrna_Neutrophil_middle$primerid, mrna_Neutrophil_early$primerid )
names(de_lists_mrna) <- c("mrna_Myeloid_late", "mrna_Myeloid_middle", "mrna_Myeloid_early","mrna_T_late", "mrna_T_middle", "mrna_T_early", "mrna_B_late", "mrna_B_middle", "mrna_B_early","mrna_Neutrophil_late", "mrna_Neutrophil_middle", "mrna_Neutrophil_early")
...
df_de$y_pos <- c(11+10,27+20,5+10,10,27,7,6,6)
...
DimPlot(immune.combined, split.by = "group" )+theme_paper
...
stats_complete <- rbind( mast_Monocyte_all,mast_B_all,mast_T_all, mast_Neutrophil_all  )

# Convert into matrix
m <-xtabs(coef~primerid+comparison, stats_complete)
attr(m, "class") <- NULL

m <- m[,c("Monocyte_early" ,"Monocyte_middle","Monocyte_late", "B_early" ,"B_middle","B_late", "T_early" ,"T_middle","T_late","Neutrophil_early" ,"Neutrophil_middle","Neutrophil_late")]

m_significance<-xtabs(fdr~primerid+comparison, stats_complete)
attr(m_significance, "class") <- NULL
m_significance <- m_significance[,c("Monocyte_early" ,"Monocyte_middle","Monocyte_late", "B_early" ,"B_middle","B_late", "T_early" ,"T_middle","T_late","Neutrophil_early" ,"Neutrophil_middle","Neutrophil_late")]
...
ggplot(de_stats_direction_plot[de_stats_direction_plot$type == "pc",], aes(x = stage_short, fill = direction))+geom_bar(stat="count", position = "dodge")+theme_paper +facet_grid(. ~ celltype)+scale_fill_manual(values = c( "#C0E4C6","#FF6666"))+ylab("Number of DE mRNAs")+xlab("")
...
FC_heatmap_subset_celltype(m, "Monocyte", unique(unlist(de_lists_lnc_my)), "Myeloid",brewer.pal(3, "Set2")[3] )
dev.off()
```
