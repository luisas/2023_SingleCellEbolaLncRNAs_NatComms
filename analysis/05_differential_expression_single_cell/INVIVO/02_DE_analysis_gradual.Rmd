...
# Output paths
robjectsdir <- file.path("/home/luisas/Desktop/cluster/data/02_scRNA-Seq_PBMCs/01_scRNA-Seq_inVivo_rhemac10/05_RObjects/04_DE_gradual/")
...
de_lists_lnc <- list(lnc_Monocyte_late$primerid,lnc_Monocyte_middle$primerid, lnc_Monocyte_early$primerid,lnc_T_late$primerid, lnc_T_middle$primerid, lnc_T_early$primerid, lnc_B_early$primerid, lnc_B_middle$primerid, lnc_B_late$primerid )
...
de_lists_mrna <- list(mrna_Monocyte_late$primerid,mrna_Monocyte_middle$primerid, mrna_Monocyte_early$primerid,mrna_T_late$primerid, mrna_T_middle$primerid, mrna_T_early$primerid, mrna_B_early$primerid, mrna_B_middle$primerid, mrna_B_late$primerid )
...
```{r colocation}
png(filename = "/home/luisas/Desktop/cluster/data/plots/B_lnc_FC_gradual.png", width = 1300, height = 1100)
...
# maximum 1Mb
all_lnc_2mb <- do.call(rbind,lapply(1:(length(lnc_ranges_complete_de)), function(index) (get_n_closest_gr(lnc_ranges_complete_de[index], mrnas_ranges, maxgap = 2000000L))))
...
ego <- clusterProfiler::enrichGO(gene = unlist(unique(all_lnc_2mb_names)),OrgDb =org.Mmu.eg.db, keyType = "SYMBOL",ont = "BP",universe =unique(rownames(immune.combined)))
...
# Check how many de protein coding genes i find near every de lnc
```{r colocation}
