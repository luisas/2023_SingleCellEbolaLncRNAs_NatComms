...
theme_paper <- theme(legend.title = element_blank())+theme(panel.background = element_rect(fill = "white", colour = "white"))+theme(panel.background = element_rect(fill = "white", colour = "black"))+theme(axis.text = element_text(size = 18), axis.title = element_text(size = 20), legend.text = element_text(size = 18))
...
de_stats_direction_plot <- de_all %>%  dplyr::group_by(primerid, celltype,type) %>% dplyr::summarise(direction = toString((direction)))
de_stats_direction_plot <- de_all %>%  dplyr::group_by(primerid, celltype,type,subtype) %>% dplyr::summarise(direction = toString((direction)))
...
# Gene 1 should go up mostly 
shared_1 <- shared_all[2]
de_all[de_all$primerid == shared_1,]

plot_expression_gene(shared_1, mono)
plot_expression_gene(shared_1, B)
plot_expression_gene(shared_1, T)
monosafe <- mono
immune.combined$celltype <- Idents(immune.combined)
immune.combined$group
mono$group
DotPlot(mono, features = shared_all[1], group.by = "group",cols = c("light grey", "dark red"))+coord_flip()+theme_bw()

gene <- shared_1
pal_celltypes <- c("#FD6467","#F1BB7B","#AE4E4E","#D67236")
col_mono <- pal_celltypes[1]
...
plot_genes_ALL_celltypes <- function(gene){
  plot_genes_celltypes(gene, mono,"monocytes")
  plot_genes_celltypes(gene, B,"B")
  plot_genes_celltypes(gene, T,"T")
}

plot_genes_ALL_celltypes(shared_all[1])
plot_genes_ALL_celltypes(shared_all[1])
plot_genes_ALL_celltypes("ENSMMUG00000058532-unknown")




test <-final_summary[final_summary$celltype == "B", ]
test[order(test$DE_coefficient, decreasing = TRUE),]

plot_genes_ALL_celltypes("ENSMMUG00000057439-unknown")

de_all[de_all$primerid =="ENSMMUG00000057439-unknown",  ]
DotPlot(mono, features  = "ENSMMUG00000057439-unknown", group.by = "group")
...
#plot_expression_gene(get_gene_id("NEAT1"))
#plot_expression_gene(get_gene_id("MALAT1"))
#plot_expression_gene("SRRM2")
