...
FC_heatmap_subset_celltype(m, "T", unique(unlist(de_lists_lnc_T)), "T",  "#A0A0A0",orthologs, lncpedia, reported_immlnc, legend = TRUE, height = 17)
dev.off()
```
...
FC_heatmap_subset_celltype(m, "B", unique(unlist(de_lists_lnc_B)), "B",  "#A0A0A0",orthologs, lncpedia, reported_immlnc, legend = TRUE, height = 20 )
dev.off()
```
...
FC_heatmap_subset_celltype(m, "Monocyte", unique(unlist(de_lists_lnc_my)), "Monocyte",  "#A0A0A0",orthologs, lncpedia, reported_immlnc, legend = TRUE, height = 40 )
dev.off()
...
de_lnc[de_lnc$celltype == "Monocyte" & de_lnc$ortholog_found == TRUE ,]
```

# ---------------------------------------------------------------
#            Specificity of DE expression - CELL TYPE 
# ---------------------------------------------------------------
```{r Upsets}
library(UpSetR)
# Cell-type
...
# ---------------------------------------------------------------
#            Specificity of DE expression - STAGE
# ---------------------------------------------------------------
```{r Upsets}
# Per celltype 

celltype <- "B"
de_spec <- de_all[, c("stage", "type", "primerid")]
de_spec_pc <- de_spec[de_spec$type == "pc", ]
de_spec <- de_spec[de_spec$type == "lnc", ]

# Early middle late
get_upset_stages <- function(de_spec){
    x = list( unlist(unique(de_spec[de_spec$stage == "early", c("primerid")])), unlist(unique(de_spec[de_spec$stage == "middle", c("primerid")])),unlist(unique(de_spec[de_spec$stage == "late", c("primerid")])))
  length(unique(unlist(x)))
  names(x)  <-  c( "early " , "middle","late" )
  upset_colored<- upset(fromList(x), order.by = "freq",text.scale = c(2.5, 2.5, 2.5, 1.0, 2, 2.15),sets.bar.color="grey",  main.bar.color=c(rep("black",3), rep("dark grey",4)))
  return(upset_colored)
}


get_upset_numbers<- function(de_spec){
  de_spec_number <- de_spec %>% dplyr::group_by(primerid) %>% dplyr::count()
  x = list( unlist(unique(de_spec_number[de_spec_number$n == 1, c("primerid")])), unlist(unique(de_spec_number[de_spec_number$n == 2, c("primerid")])),unlist(unique(de_spec_number[de_spec_number$n == 3, c("primerid")])))
  length(unique(unlist(x)))
  names(x)  <-  c( "1" , "2","3" )
  upset_colored<- upset(fromList(x),
                        text.scale = c(2.5, 2.5, 2.5, 1.0, 2, 2.15),
                        sets.bar.color="grey",
                        main.bar.color=c(rep("black",3)), 
                        sets =c( "3" , "2","1" ))
  return(upset_colored)
}
...
# -------------------  Save plots 
pdf(file.path(plots, "03/upset_stages_lnc.pdf"), width = 7, height = 5)
get_upset_stages(de_spec)
dev.off()

pdf(file.path(plots, "03/upset_stages_pc.pdf"), width = 7, height = 5)
get_upset_stages(de_spec_pc)
dev.off()

pdf(file.path(plots, "03/upset_stages_lnc_numbers.pdf"), width = 7, height = 5)
get_upset_numbers(de_spec)
dev.off()

pdf(file.path(plots, "03/upset_stages_pc_numbers.pdf"), width = 7, height = 5)
get_upset_numbers(de_spec_pc)
dev.off()
...
early_lnc <-  unlist(unique(de_spec[de_spec$stage =="early", c("primerid")])); 
early_pc <-  unlist(unique(de_spec_pc[de_spec_pc$stage =="early", c("primerid")]));

middle_lnc <-  unlist(unique(de_spec[de_spec$stage =="middle", c("primerid")])); 
middle_pc <-  unlist(unique(de_spec_pc[de_spec_pc$stage =="middle", c("primerid")]));

late_lnc <-  unlist(unique(de_spec[de_spec$stage =="late", c("primerid")])); 
late_pc <-  unlist(unique(de_spec_pc[de_spec_pc$stage =="late", c("primerid")]));

all_lnc <- unique(c(early_lnc, middle_lnc,late_lnc )); all_pc <- unique(c(early_pc, middle_pc,late_pc ));
non_stage_specific_lnc <- unique(c(intersect(early_lnc, middle_lnc), intersect(middle_lnc,late_lnc), intersect(late_lnc, early_lnc)))
stage_specific_lnc <- setdiff(all_lnc, non_stage_specific_lnc)
non_stage_specific_pc <- unique(c(intersect(early_pc, middle_pc), intersect(middle_pc,late_pc), intersect(late_pc, early_pc)))
...
m_type <- matrix(c(x11, x12,x21,x22),2,2,byrow = TRUE)
rownames(m_type) <- c("lncRNA","PC")
colnames(m_type) <- c("stage_specific","non_stage_specific")
m_type
fisher.test(m_type,alternative = "greater")
...
df <- data.frame(label = rep(c("stage specific"), 2))
df$type <- c("lnc", "pc")
df$n <- c(length(cell_type_specific_lnc)/length(all_lnc), length(cell_type_specific_pc)/length(all_pc))
df$n <- df$n*100
ggplot(df, aes(x = label, fill = type, y = n))+geom_bar(stat = "identity", position = position_dodge())+theme_paper+ylab("% cell-type specific DE genes")+xlab("")+scale_fill_manual(values= c(palette_plot_percentage))
...
# Fisher test stage specific - matche by % cells 
...
# Only retain the matched one for the test 
early_lnc <-  early_lnc[early_lnc %in% matches_perc$gene_id]; early_pc <- early_pc[early_pc %in% matches_perc$gene_id];
middle_lnc <- middle_lnc[middle_lnc %in% matches_perc$gene_id]; middle_pc <- middle_pc[middle_pc %in% matches_perc$gene_id];
late_lnc <- late_lnc[late_lnc %in% matches_perc$gene_id]; late_pc <- late_pc[late_pc %in% matches_perc$gene_id];
...
pdf(file.path(plots, "03/neatExpr.pdf"), width = 7, height = 6)
plot_expression_gene(get_gene_id("NEAT1"), mono)
dev.off()
