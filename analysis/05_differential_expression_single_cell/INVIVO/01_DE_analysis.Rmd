...
corr_ref <- distinct(data.frame(gname = ref[ref$type == "gene",]$gene_name, id = ref[ref$type == "gene",]$gene_id, stringsAsFactors = F))
corr_ref <- corr_ref[corr_ref$gname %in% summary$gene_name, ]
multiple <- corr_ref %>% dplyr::group_by(gname) %>% dplyr::tally() %>% filter(n>1)
...
orthologs <- distinct(orthologs[,c("gene_id", "orthologGeneSymbol")])
...
unique(summary[summary$ortholog_found == T, c("ortholog", "celltype")])
summary

orthologs[orthologs$orthologGeneSymbol == "GAS5",]
...
ggplot(de_stats_direction_plot[de_stats_direction_plot$type == "pc",], aes(x = stage_short, fill = direction))+geom_bar(stat="count", position = "dodge")+theme_paper +facet_grid(. ~ celltype)+scale_fill_manual(values = rev(c("red", "navy")))+ylab("# DE mRNAs")+xlab("")
dev.off()

pdf(file.path(plots, "03/SUPPL_directionalityLNC.pdf"), width = 8, height = 6)
ggplot(de_stats_direction_plot[de_stats_direction_plot$type == "lnc",], aes(x = stage_short, fill = direction))+geom_bar(stat="count", position = "dodge")+theme_paper +facet_grid(. ~ celltype)+scale_fill_manual(values = rev(c("red", "navy")))+ylab("#DE lncRNAs")+xlab("")
dev.off()
...
FC_heatmap_subset_celltype(m_expr, "Monocyte", unique(unlist(de_lists_lnc_my)), "Monocyte", "#A0A0A0",orthologs, lncpedia, reported_immlnc, legend = T )
dev.off()
```
```{r heatmapTlncExpr}
...
FC_heatmap_subset_celltype(m_expr, "T", unique(unlist(de_lists_lnc_T)), "T",  pal_celltypes[1],orthologs, lncpedia, reported_immlnc, legend = T)
dev.off()
```
```{r heatmapBlncExpr}
...
FC_heatmap_subset_celltype(m_expr, "B", unique(unlist(de_lists_lnc_B)), "B",  brewer.pal(3, "Set2")[2],orthologs, lncpedia, reported_immlnc, legend = T )
dev.off()
```
...
de_lnc$ImmLnc <- ifelse(de_lnc$ortholog %in% immlnc$lncRNA_symbol, "reported in Immlnc", "not reported in ImmLnc")
de_lnc$ImmLnc <- factor(de_lnc$ImmLnc, levels = c( "not reported in ImmLnc","reported in Immlnc"))
de_lnc_red <- de_lnc[,c("ortholog","ortholog_found", "ImmLnc")]                    

# How many DE genes have an ortholog 
de_lnc%>% dplyr::group_by(ortholog_found) %>% dplyr::summarise(n = n()) %>% dplyr::mutate(freq = n / sum(n))
# How many had been prev reported by ImmLnc
de_lnc[de_lnc$ortholog_found == TRUE, ] %>% dplyr::group_by(ImmLnc) %>% dplyr::summarise(n = n()) %>% dplyr::mutate(freq = n / sum(n))


ortholog_summary <-de_lnc[de_lnc$ortholog_found == TRUE, ] %>% dplyr::group_by( celltype,ortholog_found, ImmLnc) %>% dplyr::summarise(n = n()) %>% dplyr::mutate(freq = n / sum(n))

ggplot(ortholog_summary, aes(x = celltype, fill = ImmLnc, y = n))+geom_bar(stat = "identity")+theme_paper+scale_fill_manual(values = rev(c("#000033","grey")))+ylab("#genes")+xlab("")+ggtitle("DE genes for which an ortholog was found")

pdf(file.path(plots, "03/SUPPL_orthologs.pdf"), width = 7, height = 6)
ggplot(ortholog_summary, aes(x = celltype, fill = ImmLnc, y = freq))+geom_bar(stat = "identity")+theme_paper+scale_fill_manual(values = rev(c("#000033","grey")))+ylab("% DE genes")+xlab("")+theme_classic()+theme(axis.ticks.x = element_blank(), text = element_text(size = 20), axis.line.x = element_blank()) +theme(axis.line.y.left  = element_line(color= "black"), legend.title = element_blank())
dev.off()
...
plot <- upset(fromList(x), order.by = "freq",text.scale = c(2.5, 2.5, 2.5, 1.0, 2, 2.15),sets.bar.color=pal_celltypes[c(3,1,2)],  main.bar.color=c(rep("black",3), rep("dark grey",3)))
...
# Genes in multiple celltypes: investigate which ones
...
#orthologs[orthologs$orthologGeneSymbol.y == "GAS5",]
...
# Get the top 5  down in Monocyetes
print("up mono")
unique(unlist(lapply(lnc_de_mono[rev(order(lnc_de_mono$coef)),][1:6, ]$primerid, get_orthologname_)))

# Get the top 5  down in B
print("up T")
unique(unlist(lapply(lnc_de_t[rev(order(lnc_de_t$coef)),][1:8, ]$primerid, get_orthologname_)))

#orthologs[orthologs$orthologGeneSymbol.y == "GAS5",]

# Get the top 5  down in T
print("up b")
unique(unlist(lapply(lnc_de_b[rev(order(lnc_de_b$coef)),][1:5, ]$primerid, get_orthologname_)))

# How many are in ImmLnc? 

de_names <- unlist(lapply(lnc_de_all$primerid, function(x) get_orthologname_(x)))
length(unique(de_names))
de_orth <- de_names[!(substr(de_names,1,3) %in% c("MST", "ENS"))]

# The ones that have an ortholog, have they been reported in immlnc?
table(unique(de_orth) %in% reported_immlnc)/length(unique(de_orth))

# Not de (But expressed)
...
lnc_expressed <- (unique(df_complete[df_complete$type == "lncRNA", ]$gene_id))
lnc_expressed_notDE <- (unique(setdiff(lnc_expressed,lnc_de_all$primerid)))
lnc_expressed_notDE_orth <- unlist(lapply(lnc_expressed_notDE, function(x) get_orthologname_(x)))
lnc_expressed_notDE_orth <- lnc_expressed_notDE_orth[!(substr(lnc_expressed_notDE_orth,1,3) %in% c("MST", "ENS"))]

table(unique(lnc_expressed_notDE_orth) %in% reported_immlnc)/length(unique(lnc_expressed_notDE_orth))
```
...
p <- df %>% dplyr::group_by(mono.individual, mono.dpi) %>% dplyr::summarise(  mean = mean(value))
...
pdf(file.path(plots, "03/SUPPL_neatExpr_sepMono.pdf"), width = 7, height = 6)
plot_neat
dev.off()


plot.data_up$id <- factor(plot.data_up$id, levels = c("baseline", "early", "middle","late"))
pdf(file.path(plots, "03/neatExpr.pdf"), width = 7, height = 6)
ggplot(plot.data_up, aes(x= id, y = avg.exp))+geom_point(shape = 1,colour = "black", fill = "white", alpha = 1, stroke = 1, aes(size = pct.exp))+theme_classic()+ylab("average expression (logCP10K)")+geom_smooth(method=lm, aes(fill=id))+theme(text = element_text( size = 18))+xlab("")+geom_line(group = "features.plot")+theme(legend.title = element_blank())
dev.off()



ggplot(plot.data_up, aes(x= id, y = pct.exp))+geom_point(size = 3.5, lty =3)+theme_classic()+ylab("average expression (logCP10K)")+geom_smooth(method=lm, aes(fill=id))+theme(text = element_text( size = 18))+xlab("")+geom_line(group = "features.plot")
```


```{r expression}
mono$group <- factor(mono$group, levels = c("baseline", "early","middle", "late"))
neat1 <- get_gene_id("LINC00877")
#neat1 <- get_gene_id("OIP5-AS1")
neat1exp <- mono[neat1,]


plot_expression_gene <- function(gene){
  baseline <-neat1exp[,neat1exp$group == "baseline"]@assays$RNA@data
  early <-neat1exp[,neat1exp$group == "early"]@assays$RNA@data
  middle <-neat1exp[,neat1exp$group == "middle"]@assays$RNA@data
  late <-neat1exp[,neat1exp$group == "late"]@assays$RNA@data
  ...
  exp <- Reduce(rbind,lapply(as.character(unique(neat1exp$group)), function(x) get_average(x)))
  exp$type <- factor(exp$type, levels = c("baseline", "early", "middle", "late"))
  
  
  
  #pdf(file.path(plots, "03/neatExpr.pdf"), width = 7, height = 6)
  p <- ggplot(exp, aes(x= type, y = mean))+geom_point(shape = 1,colour = "black", fill = "white", alpha = 1, stroke = 1, aes(size = pct.exp))+theme_classic()+geom_errorbar(aes(ymax = upper, ymin = lower), width=.2)+geom_smooth(method=lm, aes(fill=type), colouer = "blue")+theme(text = element_text( size = 18))+xlab("")+geom_line(group = "type", colour = "blue")+theme(legend.title = element_blank())+ylab("average expression (logCP10K)")+theme(axis.line.x = element_blank(), axis.ticks.x = element_blank())
  #dev.off()
  

  p2 <- Dotplot(mono, features=c(neat1), group.by  = "group")
  tab <- Dotplot_data(mono, features=c(neat1), group.by  = "group")
...
lnc_orth <- unlist(lapply(all_lnc, get_orthologname_))

lnc2target <- read.csv("/home/luisas/Desktop/cluster/data/00_RawData/Lnc2Target/lncrna_target.txt", sep = "\t")
lnc2target[lnc2target$lncrna_symbol %in% lnc_orth,]

lnc2target_low <- read.csv("/home/luisas/Desktop/cluster/data/00_RawData/Lnc2Target/lncRNA_target_from_low_throughput_experiments.csv")
# From Low-throughput experiments
de_lnc_targets <- lnc2target_low[lnc2target_low$lncRNA_name_from_paper %in% lnc_orth,]

pc_in_targets <- all_pc[all_pc %in% de_lnc_targets$Target_official_symbol]
