...
saveRDS(orthologs, "/home/luisas/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation/05_orthologs/orthologs_geneid_ready.rds")
```
...
ggplot(df_de , aes(x = celltype,y=n, col = subtype, fill = celltype))+geom_bar(stat = "identity")+scale_fill_manual(values = pal_celltypes_short)+scale_color_manual(values=c( "grey", "black"))+theme_paper+ylab("Number of DE LncRNAs")+xlab("")+theme(legend.position = "")+
  geom_text(aes(y=y_pos, label=n), vjust=1.6, color="white", size=6)


ggplot(mrna_de_all %>% dplyr::group_by(type, celltype) %>% dplyr::summarise(n=n_distinct(primerid)), aes(x = celltype,y=n, col = celltype, fill = celltype))+geom_bar(stat = "identity")+scale_fill_manual(values = pal_celltypes_short)+scale_color_manual(values = pal_celltypes_short)+theme_paper+ylab("Number of DE PC genes")+xlab("")+theme(legend.position = "")+geom_text(aes(y=n, label=n), vjust=1.6, color="white", size=6)
...
FC_heatmap_subset_celltype(m, "T", unique(unlist(de_lists_lnc_T)), "T",  pal_celltypes[1],orthologs, lncpedia, reported_immlnc, legend = T)
dev.off()
```
```{r heatmapBlnc}
png(filename = "/home/luisas/Desktop/cluster/data/plots/neut/B_lnc_FC.png", width = 1300, height = 1100)
FC_heatmap_subset_celltype(m, "B", unique(unlist(de_lists_lnc_B)), "B",  brewer.pal(3, "Set2")[2],orthologs, lncpedia, reported_immlnc, legend = T )
dev.off()
```

```{r heatmapMonolnc}
png(filename = "/home/luisas/Desktop/cluster/data/plots/neut/Mono_lnc_FC.png", width = 1300, height = 1100)
FC_heatmap_subset_celltype(m, "Monocyte", unique(unlist(de_lists_lnc_my)), "Monocyte",  brewer.pal(3, "Set2")[3],orthologs, lncpedia, reported_immlnc, legend = T )
dev.off()
```
...
ggplot(ortholog_summary, aes(x = celltype, fill = ImmLnc, y = freq))+geom_bar(stat = "identity")+theme_paper+scale_fill_manual(values = rev(c("#000033","grey")))+ylab("%Genes")+xlab("")+ggtitle("DE Genes for which an ortholog was found")
...
upset(fromList(x), order.by = "freq",text.scale = c(2.5, 2.5, 2.5, 1.0, 2, 2.15),sets.bar.color=brewer.pal(3,"Set2"),  main.bar.color=c(rep("black",3), rep("dark grey",3)))
...
names(x_stages)  <-  c( "Early " , "Middle ","Late" )
