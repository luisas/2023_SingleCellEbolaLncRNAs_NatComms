...
plot_corr_gene_box <- function(gene, n){
  ...
  ggplot(df_ordered, aes(x= bin, y = exp))+geom_boxplot(fill = "#5bcfcb", alpha =0.9)+theme_paper+xlab("Viral Load")+ylab("logCP10K")+theme(axis.text.x = element_blank())+ggtitle(paste(gene, gene_gas5, sig_correlations_lnc_infected_24[sig_correlations_lnc_infected_24$gene == gene_gas5,]$rho, sep = "  "))
}


plot_corr_gene_box("GAS5", 40)
...
plot_corr <- function(expression_matrix, gene, rho){
  ...
  # Order cells by viral load 
  df_ordered <- df[order(df$viral_load),]
  ggplot(df_ordered, aes(x = viral_load, y = exp))+geom_point( size = 0.6, color = "orange")+geom_line(stat = "summary_bin", binwidth = 0.05,  color = "orange")+scale_x_continuous(trans = "log10")+theme_paper+ylim(0,4)+ggtitle(paste(gene,rho ))+geom_smooth()
}
