...
genes_negative <- c("ENSMMUG00000064224-unknown", "IFIT2", "ISG15", "MSTRG.154583-unknown", "MX1")
...
gene <- "MSTRG.181870-unknown"
gene <- "ENSMMUG00000061044-unknown"
gene <- "ENSMMUG00000058644-unknown"
gene <- "ENSMMUG00000064224-unknown"
gene <- "MSTRG.238227-unknown"
...
mono_live_h24_inf_exvivo <- mono

genes <- c("ENSMMUG00000064224-unknown", "IFIT2", "ISG15", "MX1")
  
  
  
visualize_correlation <- function(gene,mono_live_h24_inf_exvivo,ebola_genome_percentage_df_exvivo,mono, keepzeros = T){
  #genes_expression <-  Reduce("rbind", lapply(genes, function(gene) get_expression_summary_gene(gene, mono, ebola_genome_percentage_df_exvivo, T)))
  ...
  correlation <- ggplot(genes_expression, aes(x = percentage_viral_reads, y = value, col = orth))+stat_smooth(method = "loess", formula = y ~ x, n = 15, span =2)+
    theme_bw()+theme(legend.position = "none", plot.title = element_text(size = 12), text = element_text(size = 17))+ 
    xlab("% viral load")+ylab("log(CP10K+1)")+theme_paper+ ggtitle(get_orthologname_(gene))
 ...
 expression <- mono[gene,]
  ...
  exp$pct.exp_2 <- 2*(exp$pct.exp)
  
  expression_plot <- ggplot(exp, aes(x= type, y = mean,))+geom_point(shape = 1,colour = "black", fill = "white", alpha = 6, stroke = 1, aes(size = pct.exp), show.legend = T)+scale_size(range = c(5,15))+theme_classic()+geom_errorbar(aes(ymax = upper, ymin = lower), width=.2)+ylab("log(CP10K+1)")+theme(text = element_text( size = 18))+xlab("")+theme(legend.title = "% of cells", legend.position = "bottom")+theme(axis.line.x = element_blank(), axis.ticks.x = element_blank())+theme_paper+theme_paper
...
ggplot(exp, aes(x= type, y = mean))+geom_point(shape = 1,colour = "black", fill = "white", alpha = 10, stroke = 1, aes(size = pct.exp_2), show.legend = T)+theme_classic()+geom_errorbar(aes(ymax = upper, ymin = lower), width=.2)+ylab("log(CP10K+1)")+theme(text = element_text( size = 18))+xlab("")+theme(legend.title = element_blank(), legend.position = "bottom")+theme(axis.line.x = element_blank(), axis.ticks.x = element_blank())+theme_paper
  
  
  
  both <- grid.arrange(expression_plot, correlation, ncol=2, top = textGrob(get_orthologname_(gene) ,gp=gpar(fontsize=20,font=3)))
...
monocytes <- mono_live_h24_inf_exvivo_noebola
