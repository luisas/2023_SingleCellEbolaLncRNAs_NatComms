...
```{r include=FALSE}
# To be deleted: old vs new ref file 
seqnames(ref)
seqnames(ref_old)
setdiff(as.character(unique(seqnames(ref))), as.character(unique(seqnames(ref_old))))


# 
ref[grepl("poly*", ref$gene_name),]
chr1<- ref[seqnames(ref) == "1",]


chr1_old <- ref_old[seqnames(ref_old) == "1",]
ref_old[grepl("poly*", ref_old$gene_name),]
table(ref[grepl("poly*", ref$gene_name),]$type)

start = 187099
ref[start(ref) == start]
ref_old[start(ref_old) == start]
novelcon[start(novelcon) == start]
novel_concordant[start(lnc_novel_compared_to_ref) == start]
poly_old[start(poly_old) == start]
gtf_ref_and_novel_sort[start(gtf_ref_and_novel_sort) == start]

poly<- import(file.path("/home/luisas/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation/03_novel_lncRNAs_list/02_novel_expressed/novel_expressed_polya.gtf"))
ribo<- import(file.path("/home/luisas/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation/03_novel_lncRNAs_list/02_novel_expressed/novel_expressed_ribodepleted.gtf"))
polynov<- import(file.path("/home/luisas/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation/03_novel_lncRNAs_list/03_polyA_vs_ribodepl/ribodeplVSpolyA.annotated.gtf"))
table(poly$type)
table(ribo$type)
table(polynov$type)
table(ref_old[grepl("ribo*", ref_old$gene_name),]$type)
table(ref[grepl("ribo*", ref$gene_name),]$type)

poly_old <-import(file.path("/home/luisas/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation/old/03_novel_lncrnas/01_novel_expressed/novel_expressed_polyA.gtf"))

# Novel concordant 
novelcon <-import(file.path("/home/luisas/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation/old/03_novel_lncrnas/00_all_novels/novel_rhemac10_concordant_polyA.gtf"))

poly
ref[grepl("polya*", ref$gene_name),]
```
...
#immune.combined_invivo <- readRDS(file.path(robjectsdir, "INVIVO_immune.combined_merged.rds"))
