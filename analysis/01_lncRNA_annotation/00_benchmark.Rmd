...
noncoding_cpc2_transcripts <- unlist(lapply(cpc2[cpc2$label == "noncoding",]$ID, function(x) strsplit(as.character(x),'(', fixed = T)[[1]][1]))
coding_cpc2_transcripts <- unlist(lapply(cpc2[cpc2$label == "coding",]$ID, function(x) strsplit(as.character(x),'(', fixed = T)[[1]][1]))

noncoding_cpat_transcripts <- unlist(lapply(cpat[cpat$label == "noncoding",]$ID, function(x) strsplit(as.character(x),'(', fixed = T)[[1]][1]))
coding_cpat_transcripts <- unlist(lapply(cpat[cpat$label == "coding",]$ID, function(x) strsplit(as.character(x),'(', fixed = T)[[1]][1]))

noncoding_cnit_transcripts <- unlist(lapply(cnit[cnit$index == "noncoding",]$Transcript.ID, function(x) strsplit(as.character(x),'(', fixed = T)[[1]][1]))
coding_cnit_transcripts <- unlist(lapply(cnit[cnit$index == "coding",]$Transcript.ID, function(x) strsplit(as.character(x),'(', fixed = T)[[1]][1]))


# How many of the known lncRNAs (GENES) are identified as such
noncoding_cpc2 <- df_ref[df_ref$transcript_id %in% unique(noncoding_cpc2_transcripts),]$gene_id
coding_cpc2 <- df_ref[df_ref$transcript_id %in% unique(coding_cpc2_transcripts),]$gene_id


noncoding_cpat <- df_ref[df_ref$transcript_id %in% unique(noncoding_cpat_transcripts),]$gene_id
coding_cpat <- df_ref[df_ref$transcript_id %in% unique(coding_cpat_transcripts),]$gene_id

noncoding_cnit <- df_ref[df_ref$transcript_id %in% unique(noncoding_cnit_transcripts),]$gene_id
coding_cnit <- df_ref[df_ref$transcript_id %in% unique(coding_cnit_transcripts),]$gene_id
...
annotated_lnc <- unique(ref_lnc$gene_id)
