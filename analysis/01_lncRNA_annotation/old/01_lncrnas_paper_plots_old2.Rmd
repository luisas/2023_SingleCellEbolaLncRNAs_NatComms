...
all <- import(file.path("/home/luisas/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation/03_novel_lncRNAs_list_5k//rheMac10_EBOV_and_novel_genenames.gtf"))
...
```{r stats}
classes <- read.table("/home/luisas/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation/02_RNA-Seq_ribodepl/05_feelNC_5k//02_feelnc_classifier/rheMac10_EBOV-Kikwit_lncRNA_classes.txt", header = TRUE)

novel_ribo<- import("/home/luisas/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation/02_RNA-Seq_ribodepl/05_feelNC_5k//feelnc_codpot_out/candidate_lncRNA.gtf.lncRNA.gtf")

novel <-novel_ribo[substr(novel_ribo$transcript_id, 1,4) == "MSTR",]
...
print(paste("Total novel lncRNA genes: ", length(unique(novel$gene_id))))
...
slices <- c(subtypes[subtypes != 0 ],genic_antisense[genic_antisense != 0 ] )  
lbls <- c(names(subtypes[subtypes != 0 ]), names(genic_antisense[genic_antisense != 0 ]))
pie(slices, labels = lbls, main="Pie Chart of Countries")


palette <- brewer.pal(3, "Set2")
myPalette <- c(rep(palette[1], length(subtypes[subtypes != 0 ])), rep(palette[2], length(genic_antisense[genic_antisense != 0 ])))
pie(slices , labels = lbls, border="white", col=myPalette )


# ------------------------------------------------
#  StringTie merged assembly compared to output 
# ------------------------------------------------
assemblyvsref <- import("//home/luisas/Desktop/cluster/data/99_BroadAnnotation/01_stringtie_assembly_merged/01_gffCompare/merged.annotated.gtf")
# test 
length(unique(assemblyvsref$transcript_id)) == sum(table(assemblyvsref[assemblyvsref$type == "transcript",]$class_code))

transcripts <- assemblyvsref[assemblyvsref$type == "transcript",]
mask_ux <- transcripts$class_code == "u" | transcripts$class_code == "x" ;
novel_transcripts <- transcripts[mask_ux]
df <- data.frame(gene = novel_transcripts$gene_id, tr = novel_transcripts$transcript_id, code = novel_transcripts$class_code)
length(unique(novel_transcripts$gene_id))
test <- distinct(df) %>% group_by(gene) %>% dplyr::summarise(code = paste(code, collapse=", "))
test$codesum <- unlist(lapply(test$code, function(x) paste(unique(trimws(unlist(str_split(x, ",")))), collapse = ",")))

# How many novel from assembly ? 
sum(table(test$codesum))
table(test$codesum)

test

# Mine
assemblyvsref <- import("//home/luisas/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation/02_RNA-Seq_ribodepl/04b_gffCompare/merged.annotated.gtf")
# test 
length(unique(assemblyvsref$transcript_id)) == sum(table(assemblyvsref[assemblyvsref$type == "transcript",]$class_code))
...
test <- distinct(df) %>% group_by(gene) %>% dplyr::summarise(code = paste(code, collapse=", "))
# How many novel from assembly ? 
sum(table(test_our$code))
table(test_our$code)


# How many make it to the candidates ? 
candidates <- import("/home/luisas/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation/02_RNA-Seq_ribodepl/05_feelNC_prediction_old3/feelnc_gencode_linc/candidate_lncRNA.gtf")
# How many candidates ? 
length(unique(candidates$transcript_id))
length(unique(candidates$gene_id))
# Hw may candidates new ? 
candidates_new <- candidates[candidates$transcript_id %in% test$tr,]
length(unique(candidates_new$transcript_id))
length(unique(candidates_new$gene_id))
# How many candidates are pred lnc ? 
pred <- import("/home/luisas/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation/02_RNA-Seq_ribodepl/05_feelNC_prediction_old3/feelnc_gencode_linc/feelnc_codpot_out/candidate_lncRNA.gtf.lncRNA.gtf")

lnc_novel_compared_to_ref_file <- "/home/luisas/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation/02_RNA-Seq_ribodepl/04b_gffCompare/merged.annotated.gtf"
lnc_novel_compared_to_ref <- import(lnc_novel_compared_to_ref_file)

# Number of unique novel ones 
predlnc <- (unique(pred[substr(pred$gene_id, 1,4) == "MSTR",]$gene_id))
length(unique(predlnc))



length(unique(candidates$transcript_id))
predlnc_comp <- lnc_novel_compared_to_ref[lnc_novel_compared_to_ref$gene_id %in% predlnc,  ]
length(unique(predlnc_comp$gene_id))

length(unique(predlnc_comp[predlnc_comp$class_code %in% c("u", "x"),]$transcript_id))
...
df <- data.frame(gene = predlnc_comp$gene_id, tr = predlnc_comp$transcript_id, code = predlnc_comp$class_code)
test <- df[!is.na(df$code),] %>% group_by(gene) %>% dplyr::summarise(code = paste(code, collapse=", "))
table(test$code) 
candidates_new_lnc <- pred[pred$transcript_id %in% test$tr,]
length(unique(candidates_new_lnc$transcript_id))
length(unique(candidates_new_lnc$gene_id))
...
novel_ribo<- import("/home/luisas/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation/02_RNA-Seq_ribodepl/05_feelNC_5k//01_gffcompare/merged.annotated.gtf")
...
conc_ribo <- import("/home/luisas/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation/03_novel_lncRNAs_list_5k//00_all_novels/novel_rhemac10_concordant_ribodepleted.gtf")
n_conc_ribo <- table(substr(unique(conc_ribo$gene_id),1,3))[[1]]

expre_ribo <- import("/home/luisas/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation/03_novel_lncRNAs_list_5k/02_novel_expressed/novel_expressed_ribodepleted.gtf")
