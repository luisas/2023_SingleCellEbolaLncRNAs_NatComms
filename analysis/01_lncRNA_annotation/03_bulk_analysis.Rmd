...
# Transpose matrix
# Row: samples, Columns: genes 
data <- t(assays(dds)$logCPM)



# Add metadata 
metadata <- data.frame(t(as.data.frame((lapply(rownames(data), extract_info_sample_from_dir)))))
...
not_infected_ids <- c("CtrlAb", "D-13", "D00","D000")
infection_status <- ifelse(metadata$DPI %in% not_infected_ids, "not_infected", "infected")
metadata <- cbind(metadata, infection_status)
...
```{r import}
n <- ncol(metadata)+1
pca_res <- prcomp(data_annotated[,n:length(data_annotated)], scale. = TRUE)
...
autoplot(pca_res, data = data_annotated,frame = FALSE, colour = 'infection_status')+theme_classic()


plotMDS(dge, top = 500, labels = NULL, pch = 1, cex = 1,
        dim.plot = c(1,2), gene.selection = "pairwise",
        xlab = NULL, ylab = NULL, method = "logFC", prior.count = 2, plot = TRUE, col = dge$samples$dpo, legend = "all")
...
# Fix Tissue naming
dds$tissue  <- as.character(dds$tissue)
dds$tissue <- unlist(lapply(dds$tissue, function(x) str_split(x, "-")[[1]][1]))
dds$tissue <- ifelse(dds$tissue == "Wholeblood", "Blood",dds$tissue )

rld <- vst(dds)
pcaData <- plotPCA(rld, ntop = 500, returnData = TRUE, intgroup = c("condition"))
