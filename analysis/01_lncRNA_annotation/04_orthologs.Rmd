...
df_pairs <- prep_pairs(orthologs_novel)
df_pairs_red <- df_pairs[df_pairs$source != "sno_host",]
sum(df_pairs_red$value)
(sum(df_pairs_red$value)/length(unique(all_stringtie_novellnc$transcript_id)))*100

# Novel lncrnas
plot_sankey(df_pairs_red)
# Annotated lncrna
df_pairs <- prep_pairs(orthologs_lncAnnot)
df_pairs_red <- df_pairs[!(df_pairs$source %in%  c("sno_host", "mir_host_exon", "mir_host_intron")),]
(sum(df_pairs_red$value)/length(unique(all_annot_lnc$transcript_id)))*100
plot_sankey(df_pairs_red,as.character(as.integer(as.factor(df_pairs_red$source))))

df <- data.frame(table(seqnames(all[all$transcript_id %in% orthologs_lncAnnot$lnc,])))
df
ggplot(df[df$Freq > 0 , ], aes(x = Var1, y = Freq, fill = "orange"))+geom_bar(stat="identity")+theme_paper+theme(legend.position = "", axis.text.x = element_text( size =12))+labs(x = "", y = "")

table(seqnames(all))

test <- import("/home/luisas/Desktop/cluster/gene_annotations/ensembl_release100/rheMac10/Macaca_mulatta.Mmul_10.100.gtf")
test
df <- data.frame(table(seqnames(all[all$transcript_id %in% orthologs_novel$lnc,])))
ggplot(df[df$Freq > 0 , ], aes(x = Var1, y = Freq, fill = "orange"))+geom_bar(stat="identity")+theme_paper+theme(legend.position = "", axis.text.x = element_text( size =12))+labs(x = "", y = "")
