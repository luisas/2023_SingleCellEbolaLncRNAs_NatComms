...
my_color <- 'd3.scaleOrdinal() .domain(["type_a", "type_b", "type_c", "my_unique_group"]) .range(["#C60F7B","orange", "navy", "darkgrey"])'
...
saveRDS(orthologs,"/home/luisas/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation/05_orthologs/orthologs_geneid.rds")
...
orthologs[orthologs$orthologGeneSymbol == "AADACL2-AS1",c("lnc", "gene_id", "orthologGeneSymbol")]

ref[ref$gene_id %in% c("ENSMMUG00000001664","ENSMMUG00000060077") & ref$type == "gene",]
```
...
# Plot
plot_sankey(df_pairs_red_novel, as.character(as.integer(as.factor(df_pairs_red_novel$source))))
...
ggplot(df, aes(x = type, fill = orth, y = freq))+geom_bar(stat = "identity", alpha  = 0.6, position = "stack")+theme_paper+xlab("")+scale_fill_manual(values = c("grey","#525252"))+ylab("Proportion")
