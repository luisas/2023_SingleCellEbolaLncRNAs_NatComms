...
# Define palettes 
palette <- c("navy", "#8195D7","#ADD8E5", "#519F78","#B4D6C1")
palette_border <- c("#C60F7B","orange", "#8195D7","#ADD8E5", "#519F78","#B4D6C1")
palette_extensive <- c("navy", "navy", "#8195D7","#ADD8E5", "#519F78","#B4D6C1")
...
type <- c("Protein Coding", "lncRNAs", "lncRNAs")
subtype <- c("Protein coding", "Annotated", "Novel")

df <- data.frame(type = type, subtype = subtype, n = n_genes)
df$subtype <- factor(df$subtype, levels = c("Novel", "Annotated", "Protein coding"))
ylab <- seq(5,20,5)
ggplot(df, aes(x = type, y = n, fill = subtype))+geom_bar(stat="identity", size = 2)+theme_paper+scale_y_continuous( labels = paste0(ylab, "K"),breaks = 10^3 * ylab)+ylab("")+xlab("")+coord_flip()+scale_fill_manual(values = palette_expression)+theme(legend.position = "")+ggtitle("Genes in annotation")
```


# Separate by intergenic and antisense 
```{r cpc2vscpan}
gffcompare <- import("/home/luisas/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation/02_RNA-Seq_ribodepl/04_stringtie_gffcompare/merged.annotated.gtf")
...
l_sep <- plot_stats_annotation_separated(intergenic_lnc,antisense_lnc ,lncRNAs_ref,lncRNAs_ref_human, mrna_ref_human, mRNAs_ref, palette_extensive, palette_border)
...
plot_expression(median_expression, palette_expression_extensive, level = levels, title = "Median Expression", palette_expression_border,sizes = c(1,1,0.5,0.5))+theme(plot.title = element_text(size = 22), axis.text = element_text( angle =0, vjust = 0.9),axis.title.y  = element_text(size = 15))+labs(y = "logTPM")
