...
df$subtype <- factor(df$subtype, levels = c("Novel", "Annotated", "mRNA"))
ylab <- seq(0,25,5)
p_summary <- ggplot(df, aes(x = type, y = n, fill = subtype))+geom_bar(stat="identity", size = 2)+xlab("")+ylab("number of genes")+coord_flip()+
        scale_fill_manual(values = palette_expression)+theme(legend.position = "")+
...
pdf(file.path(plots, "01/exon_length.pdf"), width = 4, height = 6)
l[[2]]
dev.off()

pdf(file.path(plots, "01/transcript_length.pdf"), width = 4, height = 6)
l[[3]]
dev.off()
```
...
pdf(file.path(plots, "01/MULTIEXONIC_exon_length.pdf"), width = 4, height = 6)
l_multiexonic[[2]]
dev.off()

pdf(file.path(plots, "01/MULTIEXONIC_transcript_length.pdf"), width = 4, height = 6)
l_multiexonic[[3]]
dev.off()
```
...
# I need 1 to 1 correspondence 

# Novel lncRNAs in macaque with human ortholog
novel_with_ortholog_gene_id <- unique(orthologs[orthologs$gene_id %in% unique(all_novel_lnc$gene_id),]$gene_id)
length(unique(novel_with_ortholog_gene_id))
novel_with_ortholog_gr <- all_novel_lnc[all_novel_lnc$gene_id %in% novel_with_ortholog_gene_id,]


# Humans to novel 
transcripts_human_novelortholog <- as.character(orthologs[orthologs$gene_id %in% novel_with_ortholog_gene_id, ]$ortholog)
...
plots_sep <- plot_stats_annotation_separated(intergenic_lnc,antisense_lnc ,lncRNAs_ref,lncRNAs_ref_human, mrna_ref_human, mRNAs_ref, palette_extensive, palette_border)
plots_sep
```
...
pdf(file.path(plots, "01/SUPPL_exon_length.pdf"), width = 4, height = 6)
plots_sep[[2]]
dev.off()

pdf(file.path(plots, "01/SUPPL_transcript_length.pdf"), width = 4, height = 6)
plots_sep[[3]]
dev.off()
```
...
### OLD:  investigate and find some example for plotting
```{r tissuespecificity}
ref <- all
orthologs <- readRDS("/home/luisa/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation/05_orthologs/orthologs_geneid.rds")
...
summarized_expression <- t(rowsum(t(expressed_booleans), colnames(expressed_booleans)))
# One tissue expressed o
summarized_expression_df <- data.frame(summarized_expression)
# novel only 
novels <- summarized_expression_df[substr(rownames(summarized_expression_df),1,3) == "MST",]

tissue <- "Blood"
expressed_in_tissue <- rownames(novels)[novels[,tissue] > 2]
not_expressed_in_others <- rownames(novels)[rowSums(novels[,setdiff(colnames(novels),tissue)]) <2]
candidates <- intersect(expressed_in_tissue, not_expressed_in_others)

candidates <- expressed_in_tissue
candidates_orth <- candidates[unlist(lapply(candidates, function(gene) get_orthologname_(gene) != gene))]

candidates_orth_df <- data.frame(candidate = candidates_orth, stringsAsFactors = F)
candidates_orth_df$ortholog <- unlist(lapply(candidates_orth, function(gene) get_orthologname_(gene)))
candidates_orth_df
...
gene <- candidates_orth_df[candidates_orth_df$ortholog == "ZFAS1",]$candidate
expression <- dat
expression <- as.matrix(expression)
colnames(expression) <- abundance_files
names(expression[gene,expression[gene, ]> 1] )
all[all$gene_id == gene, ]
gene_name <- unique(all[all$gene_id == gene, ]$gene_name)


plot_region(gene_name, palette_plot_percentage = c(rep(palette_expression[3], 2), rep( palette_expression[1],2)))
