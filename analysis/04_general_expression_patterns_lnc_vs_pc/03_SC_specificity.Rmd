...
#       Read in permutation file
# -----------------------------
perm <- readRDS("/home/luisas/Desktop/cluster/data/02_scRNA-Seq_PBMCs/01_scRNA-Seq_inVivo_rhemac10/05_RObjects/05_stats/00_specificity_permutation_prop/Permutations_1.rds")
# Permuted
perm <- get_df_summary(perm)
perm <- perm[perm$score != Inf,]


spec_min_corrected_new <- readRDS("/home/luisas/Desktop/cluster/data/02_scRNA-Seq_PBMCs/01_scRNA-Seq_inVivo_rhemac10/05_RObjects/05_stats/00_specificity_permutation_prop/specificity_real.rds")
spec_min_corrected_new_df <- get_df_summary(spec_min_corrected_new)


# How many lnc do i find in the object 
spec_res <- df_test <-spec_min_corrected_new_df
df_test$score_range <- range01(df_test$score)
df_test$log <- log(df_test$score)
df_test
...
ggscatter(df_tot, x = "score", y = "perc_cells", 
          add = "reg.line", conf.int = T, 
          cor.coef = TRUE, cor.method = "spearman", size=0.3, col = "#999999",add.params = list(color = "red", size = 0.3))+theme_sc+labs(x = "Specificity Score", y = "Number of cells")+theme_paper
