...
stats_specificity <- function(specificity_scores, plots = F){
  ...
  p1b <- ggboxplot(df_tot, x = "type", y = "score",  col="type", fill = "type", alpha= 0.5, short.panel.labs = T) +
...
tau <-  readRDS("/home/luisas/Desktop/cluster/data/02_scRNA-Seq_PBMCs/01_scRNA-Seq_inVivo_rhemac10/05_RObjects/05_stats/00_specificity/03_specificity_tau.rds")
colnames(tau) <- gsub("tau", "score", colnames(tau))
tau$gene <- rownames(tau)

# Plot Stats
stats_specificity(tau, plots = F)
match_plots(get_df_summary(specificity_scores))

# Visualize some of the examples 
# lnctau <- specificity_scores[all_lncrnas,]
# FeaturePlot(immune.combined, rownames(lnctau[(order(lnctau$score, decreasing =T)),][4,]), order  = T )

specificity_scores_summary <- get_df_summary(tau)
specificity_scores_summary$permutation <- "Real"
specificity_scores_summary$type <- ifelse(specificity_scores_summary$type == "lnc", "lncRNAs", "PC genes")

ggscatter(specificity_scores_summary, x = "score", y = "perc_cells_expressing",  conf.int = T, 
            cor.coef = TRUE, cor.method = "spearman", size=0.3, col = "#999999",add.params = list(color = "red", size = 0.3))+theme_sc+labs(x = "Specificity Score", y = "Number of cells")+theme_paper
...
specificity_scores <- readRDS("/home/luisas/Desktop/cluster/data/02_scRNA-Seq_PBMCs/01_scRNA-Seq_inVivo_rhemac10/05_RObjects/05_stats/00_specificity/04_specificity_alternativescore.rds")
...
# Plot Stats
stats_specificity(specificity_scores, plots = F)
match_plots(get_df_summary(specificity_scores))
...
plot_matching_scatter <- function(matches_perc){
    scatter_plot_matching <- ggplot(matches_perc, aes(x=score, y =perc_cells_expressing, col = factor(type)))+
                            ...
                            theme_matching+labs(x = "Specificity Score", y = "% Cells expressing")
