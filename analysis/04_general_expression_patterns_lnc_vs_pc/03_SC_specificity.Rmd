...
match_plots <- function(df_test, title = "specificity score"){
  ...
  # Pure boxplot
  scores_boxplot_matched <- ggboxplot(matches_perc, x = "type", y = "score",
   color = "black", fill = "type", 
   palette = palette_plot_percentage)+xlab("")+ylab(title)+theme(text = element_text(size = 20))+theme(axis.line.x = element_blank(), axis.ticks.x = element_blank(), axis.text.x =  element_blank() )+theme(legend.position = "")
 ...
 #           scale_color_manual(values = palette_plot_percentage)+theme(panel.background = element_rect(fill = "white", colour = "grey50"))+theme(axis.text = element_text(size = 15), axis.title = element_text(size = 22), legend.text = element_text(size = 18))+theme(legend.position = "")+ylab("Prop cells")
  ...
  return(list(p, p5, test1, test2, scores_boxplot_matched))
}
...
pdf(file.path(plots, "02/SPECIFICITY/SUPPL_spec_matching_cells_tau.pdf"), width = 6, height = 6)
match_plots(get_df_summary(tau), title  = "repurposed tau score")[[1]]
dev.off()

pdf(file.path(plots, "02/SPECIFICITY/SUPPL_sepc_matching_score_tau.pdf"), width = 6, height = 6)
...
pdf(file.path(plots, "02/SPECIFICITY/SUPPL_spec_tau_boxplotmarkers.pdf"), width = 3, height = 4)
stats_tau[[1]]
dev.off()
...
pdf(file.path(plots, "02/SPECIFICITY/SUPPL_spec_alt_boxplotmarkers.pdf"), width = 3, height = 4)
alt_stats[[1]]
dev.off()

plots_ups <- match_plots(get_df_summary(specificity_scores), title = "upsilon")

pdf(file.path(plots, "02/SPECIFICITY/SUPPL_spec_matching_cells_alt.pdf"), width = 6, height = 6)
plots_ups[[1]]
dev.off()

pdf(file.path(plots, "02/SPECIFICITY/SUPPL_sepc_matching_score_alt.pdf"), width = 6, height = 6)
plots_ups[[2]]
dev.off()
