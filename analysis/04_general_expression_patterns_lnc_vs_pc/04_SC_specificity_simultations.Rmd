...
mm <- matrix(df_celltype$meanexpr, ncol = length(unique(df_celltype$ident)), byrow = T, dimnames = list(unique(df_celltype$gene_id), unique(df_celltype$ident)) )
mm
df_celltype$gene_id <- as.character(df_celltype$gene_id)

tidyr::spread(df_celltype, gene_id, ident, fill = 0)

data.matrix(df_celltype[,c("gene_id","meanexpr", "ident")])
df_celltype[df_celltype$gene_id == "AAAS",]
```
