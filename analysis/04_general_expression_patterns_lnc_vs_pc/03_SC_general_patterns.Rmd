...
#ref<- import(file.path("/home/luisas/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation_old/03_novel_lncRNAs_list/rheMac10_EBOV_and_novel_genenames.gtf"))

ref_old<- import(file.path("/home/luisas/Desktop/cluster/data/01_bulk_RNA-Seq_lncRNAs_annotation_old/03_novel_lncRNAs_list_2/OLD_rheMac10_EBOV_and_novel_genenames.gtf"))
...
immune.combined$a <- "a"
DimPlot(immune.combined, reduction = "umap", group.by = "a", cols = "grey")+theme_minimal()+ theme(panel.background = element_rect(fill = "white", colour = "grey50"), panel.grid.major = element_blank(), panel.grid.minor = element_blank())+theme(legend.position = "")
...
subset(Idents(immune.combined), subset = Idents == 'NK') 
immune.combined$cell <- Idents(immune.combined)
a <- "NK"


immune.combined$cell == rep(a, length(immune.combined$cell)
type(a)
subset(Idents(immune.combined), subset = immune.combined$cell == as.character(a))
...
FeaturePlot(immune.combined, features = b[2])+theme_minimal()+ theme(panel.background = element_rect(fill = "white", colour = "grey50"), panel.grid.major = element_blank(), panel.grid.minor = element_blank())+theme(legend.position = "")

FeaturePlot(immune.combined, features = monocytes[2])+theme_minimal()+ theme(panel.background = element_rect(fill = "white", colour = "grey50"), panel.grid.major = element_blank(), panel.grid.minor = element_blank())+theme(legend.position = "")



FeaturePlot(immune.combined, features = nk[1])+theme_minimal()+ theme(panel.background = element_rect(fill = "white", colour = "grey50"), panel.grid.major = element_blank(), panel.grid.minor = element_blank())+theme(legend.position = "", )
plot_genes(immune.combined,t,threshold= 3, col = "purple")
...
design(dds)
r1 <- results(dds)
res <- results(dds, name=c("celltype_Myeloid_vs_B"))
res
