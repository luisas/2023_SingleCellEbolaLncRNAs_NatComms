...
tauExp <- calc_tau(m_exp)
tauExp <- data.frame(gene = as.character(names(tauExp)), score = tauExp, stringsAsFactors = F)
...
ggplot(specificity_scores[order(specificity_scores$score),], aes(y = score, x = gene, col = label_full ))+geom_point(size = 6)+theme_paper+theme(axis.text.x = element_blank(), axis.ticks = element_blank())+scale_color_manual(values=p)
```
...
get_score_prop_mod <- function(df_celltype, gene){
  ...
  calc_score_1  <- function(df_celltype_gene, ident){
    ...
    s <- (df_celltype_gene_celltype$obs_prop -df_celltype_gene_celltype$prop)*100/100-100*df_celltype_gene_celltype$prop
...
ggplot(specificity_scores_filt[order(specificity_scores_filt$score_ranged),], aes(y = score_ranged, x = gene, col = label_full ))+geom_point(size = 6)+theme_paper+theme(axis.text.x = element_blank(), axis.ticks = element_blank())+scale_color_manual(values=p)+ggtitle("Ranged")

```
```{r matchmedian}
library(dplyr)
df_celltype$observed <- df_celltype$n_cells
#df_celltype$ident<- df_celltype$Celltype
...
get_scores_prop <- function(df_celltype){
  specificity_scores <- Reduce("rbind", lapply(unique(df_celltype$gene_id), function(gene) get_score_prop_mod(df_celltype, gene)))
  df_labels <- data.frame(label_full = labels_full, gene = rownames(m), stringsAsFactors = F)
  specificity_scores <- merge(specificity_scores, df_labels, by="gene")
  specificity_scores$gene <- factor(specificity_scores$gene, levels = specificity_scores$gene[rev(order(specificity_scores$score))])
  return(specificity_scores)
}

specificity_scores <- get_scores_prop(df_celltype)



ggplot(specificity_scores[order(specificity_scores$score),], aes(y = score, x = gene, col = label_full ))+geom_point(size = 6)+theme_paper+theme(axis.text.x = element_blank(), axis.ticks = element_blank())+scale_color_manual(values=p)+ggtitle("Ranged")
```
...
perm <- readRDS("/home/luisas/Desktop/cluster/data/02_scRNA-Seq_PBMCs/04_simulation/02_specificity_prop/Permutations_prop_1.rds")
specificity_scores <- readRDS("/home/luisas/Desktop/cluster/data/02_scRNA-Seq_PBMCs/04_simulation/02_specificity_prop/01_specificity_prop.rds")
...
ggplot(specificity_scores[order(specificity_scores$score),], aes(y = score, x = gene, fill = label_full ))+geom_bar(stat="identity", color = "white")+theme_paper+theme(axis.text.x = element_blank(), axis.ticks = element_blank())+scale_fill_manual(values=p)
...
perm <- readRDS("/home/luisas/Desktop/cluster/data/02_scRNA-Seq_PBMCs/04_simulation/01_specificity_fc//Permutations_fc_1.rds")
specificity_scores <- readRDS("/home/luisas/Desktop/cluster/data/02_scRNA-Seq_PBMCs/04_simulation/01_specificity_fc//00_specificity_fc.rds")
...
spec_chi <- readRDS("/home/luisas/Desktop/cluster/data/02_scRNA-Seq_PBMCs/04_simulation/03_spec_chi/spec_chi.rds")
spec_chi$score <- spec_chi$chi
spec_chi$label <- labels
spec_chi$label_prop <- labels_prop
spec_chi$label_full <- labels_full
spec_chi$gene <- factor(spec_chi$gene, levels = spec_chi$gene[rev(order(spec_chi$chi))])

ggplot(spec_chi[order(spec_chi$score),], aes(y = score, x = gene, col = label_full ))+geom_point(size = 6)+theme_paper+theme(axis.text.x = element_blank(), axis.ticks = element_blank())+scale_color_manual(values=p)
