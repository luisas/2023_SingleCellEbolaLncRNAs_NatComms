...
# Create object 
pbmc33k.data <- Read10X(file.path(data_path,"00_RawData/sc_external/33kPBMCShuman/filtered_gene_bc_matrices/hg19/"))
pbmc33k <- CreateSeuratObject(pbmc33k.data, min.cells = 3, min.genes = 200, project = "10X_PBMC")
mito.genes <- grep("^MT-", rownames(pbmc33k@assays$RNA@data), value = T)
percent.mito <- colSums(expm1(pbmc33k@assays$RNA@data[mito.genes, ])) / colSums(expm1(pbmc33k@assays$RNA@data))
...
pbmc33k <- subset(pbmc33k, subset.name = "nGene", accept.high = 2500)
...
save(pbmc33k, file = file.path(data_path, "~/Projects/datasets/pbmc33k/pbmc33k_tutorial.Robj")
