...
length(unique(novel_transcript_ids))

length(unique(lnc_novel_compared_to_ref[mask_ux]$gene_id
))


length(unique(novel_transcript_ids))
length(unique(lnc_pred_file$transcript_id))


# i find all of the in the gff compare 
novel <- lnc_pred_file[lnc_pred_file$transcript_id %in% lnc_novel_compared_to_ref$transcript_id,]
